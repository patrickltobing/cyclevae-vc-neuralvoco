...
#min_idx_cycvae=98 #for raw cyclevae
min_idx_cycvae=80
#min_idx_cycvae=1

#min_idx=89 #for cyclevae with post-net
#min_idx=1
if [ $mdl_name_post == none ]; then
    min_idx=
else
    min_idx=86 #for cyclevae with post-net
    #min_idx=1
fi

#min_idx_wave=360 #for wavernn model
min_idx_wave=1

#min_idx_wave_ft=360 #for fine-tuned wavernn model
min_idx_wave_ft=1
...
if [ $mdl_name_post == "post_cycmelspxlf0capvae-laplace" ]; then
...
if [ $mdl_name_post == "post_cycmelspxlf0capvae-laplace" ]; then
    setting=${mdl_name_post}_${data_name}_lr${lr}_bs${batch_size}_bsu${batch_size_utt}_bsue${batch_size_utt_eval}_lat${lat_dim}_late${lat_dim_e}_hue${hidden_units_enc}_hle${hidden_layers_enc}_hud${hidden_units_dec}_hld${hidden_layers_dec}_hlf${hidden_layers_lf0}_hlp${hidden_layers_post}_hup${hidden_units_post}_kse${kernel_size_enc}_dse${dilation_size_enc}_ksd${kernel_size_dec}_ksf${kernel_size_lf0}_ksp${kernel_size_post}_dsd${dilation_size_dec}_dsf${dilation_size_lf0}_rse${right_size_enc}_dsp${dilation_size_post}_rsd${right_size_dec}_rsf${right_size_lf0}_rsp${right_size_post}_do${do_prob}_ep${epoch_count}_mel${mel_dim}_nhcyc${n_half_cyc}_s${spkidtr_dim}
fi
    ...
    if [ $mdl_name_post == "post_cycmelspxlf0capvae-laplace" ];then
        ...
        if [ $idx_resume -gt 0 ]; then
            ${cuda_cmd} ${expdir}/log/train_resume-${idx_resume}.log \
                train_post-gru-cycle-melsp-x-lf0cap-vae-laplace_noar.py \
                    --feats ${feats} \
                    --feats_eval_list $feats_list_eval_list \
                    --stats data/${trn}/stats_jnt.h5 \
                    --expdir ${expdir} \
        ...
        if true; then
                ...
                if [ $mdl_name_post == "post_cycmelspxlf0capvae-laplace" ];then
                    ...
                    ${cuda_cmd} ${expdir}/log/decode_rec-cycrec_${spk_trg}_${min_idx_cycvae}-${min_idx}.log \
                        calc_rec-cycrec-gv_post-gru-cycle-melspxlf0capvae-laplace_noar.py \
                            --feats ${feats_scp} \
                            --spk ${spk_trg} \
                            --outdir ${outdir} \
                            --model ${model} \
    ...
    if [ $mdl_name_post == "post_cycmelspxlf0capvae-laplace" ]; then
        if [ $spkidtr_dim != "0" ]; then
            ${cuda_cmd} ${expdir}/log/decode_dev_${min_idx_cycvae}-${min_idx}_${spkr}-${spk_trg}-${n_interp}.log \
                decode_post-gru-cycle-melspxlf0capvae-laplace_noar.py \
                    --feats ${feats_scp} \
                    --spk_trg ${spk_trg} \
                    --outdir ${outdir} \
                    --model ${model} \
...
for spk_trg in ${spks_trg_dec[@]};do
        if [ $mdl_name_wave == "wavernn_dualgru_compact_lpc_mband_16bit" ] \
            || [ $mdl_name_wave == "wavernn_dualgru_compact_lpc_mband_9bit" ]; then
            #outdir=${expdir_wave_ft}/${mdl_name_post}-${mdl_name_wave}-${data_name}_dev-${hidden_units_wave}-${epoch_count_wave}-${lpc}-${n_bands}-${batch_size_wave}-${min_idx_cycvae}-${min_idx}-${min_idx_wave}-${min_idx_wave_ft}
            outdir=${expdir_wave_ft}/${mdl_name_post}-${mdl_name_wave}-${data_name}_tst-${hidden_units_wave}-${epoch_count_wave}-${lpc}-${n_bands}-${batch_size_wave}-${min_idx_cycvae}-${min_idx}-${min_idx_wave}-${min_idx_wave_ft}
        fi
