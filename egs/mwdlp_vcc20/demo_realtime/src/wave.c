...
short read_write_wav_band(FILE *fin, FILE *fout, int n_bands) {
    ...
    // divide by number of bands
    header.overall_size /= n_bands;
    printf("(5-8) Overall size band: bytes:%u, Kb:%u \n", header.overall_size, header.overall_size/1024);
    ...
    // divide by number of bands
    header.sample_rate /= n_bands;
    printf("(25-28) Sample rate band: %u Hz\n", header.sample_rate);
    // convert to big endian
    ...
    printf("(29-32) Byte Rate Band: %u B/s , Bit Rate:%u b/s\n", header.byterate, header.byterate*8);
    fwrite(buffer4, sizeof(buffer4), 1, fout);
    ...
    short num_reflected_right_edge_samples = (num_samples - RIGHT_SAMPLES) % FRAME_SHIFT;
    ...
    header.data_size /= n_bands;
    printf("(41-44) Size of data chunk to be written Band: %u B \n", header.data_size);
...
int read_feat_write_wav_band(FILE* fin, FILE* fout, int bin_flag, int n_bands) {
    ...
    fseek(fin, 0, SEEK_SET);
    long num_samples_frame = num_frame * FRAME_SHIFT;
    long num_samples_frame_band = num_samples_frame / n_bands;
    printf("number of frames -- samples -- band: %ld -- %ld -- %ld \n", num_frame, num_samples_frame, num_samples_frame_band);
    ...
    header.data_size = (num_samples_frame_band * header.channels * header.bits_per_sample) / 8;
    ...
    unsigned int sample_rate_band = header.sample_rate / n_bands;
    fwrite(&sample_rate_band, sizeof(sample_rate_band), 1, fout);
    printf("(25-28) Sample rate -- band: %u Hz -- %u Hz\n", header.sample_rate, sample_rate_band);
    
    header.byterate = (header.sample_rate * header.bits_per_sample * header.channels) / 8;
    unsigned int byterate_band = header.byterate / n_bands;
    fwrite(&byterate_band, sizeof(byterate_band), 1, fout);
    printf("(29-32) Byte Rate -- band: %u B/s -- %u B / s , Bit Rate:%u b/s -- %u b/s\n",
        header.byterate, header.byterate*8, byterate_band, byterate_band*8);
    ...
    printf("Number of samples in processed output wav -- band: %lu -- %lu \n", num_samples_frame, num_samples_frame_band);
    printf("(41-44) Size of data chunk to be written: %u B \n", header.data_size);
    fwrite(&header.data_size, sizeof(header.data_size), 1, fout);
