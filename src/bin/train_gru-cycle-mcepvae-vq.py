...
from vcneuvoco import nn_search_batch
...
def save_checkpoint(checkpoint_dir, model_encoder, model_vq, model_decoder,
...
def main():
    ...
    def zero_feat_pad(x): return padding(x, args.pad_len, value=0.0)
    ...
    if args.resume is None and args.pretrained is None:
        ...
        init = True
        init = False
        if not check_hdf5(args.stats, "/vq_"+str(args.lat_dim)+"-"+str(args.ctr_size)):
            init =True
        if init:
            with torch.no_grad():
                while True:
                    ...
                    f_es = f_ss+f_bs
                    f_ss_first_pad_left = f_ss-model_encoder.pad_left
                    f_es_first_pad_right = f_es+model_encoder.pad_right
                    ...
                    if f_ss > 0:
                        ...
                        if args.ar_enc:
                            _, z, h_z, yz_in = model_encoder(batch_feat, yz_in, h=h_z)
                        else:
                            _, z, h_z = model_encoder(batch_feat, h=h_z)
                    else:
                        if args.ar_enc:
                            yz_in_ = torch.zeros((batch_feat.shape[0], 1, n_spk+args.lat_dim)).cuda()
                            _, z, h_z, yz_in = model_encoder(batch_feat, yz_in_)
                        else:
                            _, z, h_z = model_encoder(batch_feat)
                ...
                write_hdf5(args.stats, "/vq_"+str(args.lat_dim)+"-"+str(args.ctr_size), centroids)
                logging.info(kmeans.inertia_)
                unique, counts = np.unique(labels, return_counts=True)
                cluster_stats = dict(zip(unique, counts))
                logging.info(cluster_stats)
                logging.info("K-means clustering in %.3f sec" % (time.time() - start))
        else:
            centroids = read_hdf5(args.stats, "/vq_"+str(args.lat_dim)+"-"+str(args.ctr_size))
    ...
    iter_idx = 0
    min_idx = -1
    #min_eval_loss_mcd_src_trg = 6.206581
    #min_eval_loss_mcd_src_trg_std = 0.484707
    #iter_idx = 82431
    #min_idx = 38
    ...
    while epoch_idx < args.epoch_count:
        ...
        if c_idx < 0: # summarize epoch
            ...
            while True:
                with torch.no_grad():
                    ...
                    if f_ss > 0:
                        ...
                        for i in range(0,n_half_cyc_eval,2):
                            ...
                            if len(del_index_utt) > 0:
                                ...
                                if args.ar_dec:
                                    ...
                                    x_prev[i] = torch.FloatTensor(np.delete(x_prev[i].cpu().data.numpy(), \
                                                                    del_index_utt, axis=0)).to(device)
                                    x_prev_cv[i_cv] = torch.FloatTensor(np.delete(x_prev_cv[i_cv].cpu().data.numpy(), \
                                                                    del_index_utt, axis=0)).to(device)
                                ...
                                if n_half_cyc_eval > 1:
                                    ...
                                    if args.ar_dec:
                                        ...
                                        x_prev[j] = torch.FloatTensor(np.delete(x_prev[j].cpu().data.numpy(), \
                                                                        del_index_utt, axis=0)).to(device)
                            if i > 0:
                                idx_in += 1
                                if args.ar_enc:
                                    qy_logits[i], qz_alpha[i],  h_z[i], yz_in[i] \
                                        = model_encoder(torch.cat((batch_feat_in[idx_in][:,:,:args.excit_dim], batch_mcep_rec[i-1]), 2), yz_in[i], h=h_z[i], outpad_right=outpad_rights_eval[idx_in])
                                else:
                                    qy_logits[i], qz_alpha[i], h_z[i] \
                                        = model_encoder(torch.cat((batch_feat_in[idx_in][:,:,:args.excit_dim], batch_mcep_rec[i-1]), 2), h=h_z[i], outpad_right=outpad_rights_eval[idx_in])
                                i_1 = i-1
                                idx_in_1 = idx_in-1
                                ...
                                if args.ar_enc:
                                    qy_logits[i], qz_alpha[i], h_z[i], yz_in[i] = model_encoder(batch_feat_in[idx_in], yz_in[i], h=h_z[i], outpad_right=outpad_rights_eval[idx_in])
                                else:
                                    qy_logits[i], qz_alpha[i], h_z[i] = model_encoder(batch_feat_in[idx_in], h=h_z[i], outpad_right=outpad_rights_eval[idx_in])
                            idx_vq_z[i] = nn_search_batch(qz_alpha[i], model_vq.weight)
                            z[i] = model_vq(idx_vq_z[i])
                            idx_in += 1
                            if args.ar_dec:
                                batch_mcep_rec[i], h_mcep[i], x_in[i] = model_decoder(batch_sc_in[idx_in], z[i], x_in[i], h=h_mcep[i], outpad_right=outpad_rights_eval[idx_in])
                                batch_mcep_cv[i_cv], h_mcep_cv[i_cv], x_in_cv[i_cv] = model_decoder(batch_sc_cv_in[i_cv], z[i], x_in_cv[i_cv], h=h_mcep_cv[i_cv], outpad_right=outpad_rights_eval[idx_in])
                            else:
                                if args.diff:
                                    batch_mcep_rec[i], h_mcep[i], x_prev[i] = model_decoder(batch_sc_in[idx_in], z[i], x_prev=x_prev[i], h=h_mcep[i], outpad_right=outpad_rights_eval[idx_in])
                                    batch_mcep_cv[i_cv], h_mcep_cv[i_cv], x_prev_cv[i_cv] = model_decoder(batch_sc_cv_in[i_cv], z[i], x_prev=x_prev_cv[i_cv], h=h_mcep_cv[i_cv], outpad_right=outpad_rights_eval[idx_in])
                                else:
                            ...
                            if args.ar_enc:
                                qy_logits[j], qz_alpha[j], h_z[j], yz_in[j] \
                                    = model_encoder(torch.cat((batch_feat_cv_in[i_cv], batch_mcep_cv[i_cv]), 2), yz_in[j], h=h_z[j], outpad_right=outpad_rights_eval[idx_in])
                            else:
                                qy_logits[j], qz_alpha[j], h_z[j] \
                                    = model_encoder(torch.cat((batch_feat_cv_in[i_cv], batch_mcep_cv[i_cv]), 2), h=h_z[j], outpad_right=outpad_rights_eval[idx_in])
                            ...
                            if n_half_cyc_eval > 1:
                                idx_in += 1
                                if args.ar_dec:
                                    batch_mcep_rec[j], h_mcep[j], x_in[j] = model_decoder(batch_sc_in[idx_in], z[j], x_in[j], h=h_mcep[j], outpad_right=outpad_rights_eval[idx_in])
                                else:
                                    if args.diff:
                                        batch_mcep_rec[j], h_mcep[j], x_prev[j] = model_decoder(batch_sc_in[idx_in], z[j], x_prev=x_prev[j], h=h_mcep[j], outpad_right=outpad_rights_eval[idx_in])
                                    else:
                        ...
                        if args.ar_enc:
                            _, trj_lat_src_, _, _ = model_encoder(batch_feat_data_full, yz_in_)
                        else:
                            _, trj_lat_src_, _ = model_encoder(batch_feat_data_full)
                        ...
                        if args.ar_dec:
                            trj_src_src, _, _ = model_decoder(batch_sc_data_full, trj_lat_src, x_in_)
                            trj_src_trg, _, _ = model_decoder(batch_sc_cv_data_full, trj_lat_src, x_in_)
                        else:
                            if args.diff:
                                trj_src_src, _, _= model_decoder(batch_sc_data_full, trj_lat_src, x_prev=x_in_)
                                trj_src_trg, _, _ = model_decoder(batch_sc_cv_data_full, trj_lat_src, x_prev=x_in_)
                            else:
                        ...
                        if pair_flag:
                            ...
                            if args.ar_enc:
                                _, trj_lat_trg_, _, _ = model_encoder(batch_feat_trg_data_in, yz_in_)
                            else:
                                _, trj_lat_trg_, _ = model_encoder(batch_feat_trg_data_in)
                        ...
                        for i in range(0,n_half_cyc_eval,2):
                            ...
                            if i > 0:
                                ...
                                if args.ar_enc:
                                    qy_logits[i], qz_alpha[i], h_z[i], yz_in[i] \
                                        = model_encoder(torch.cat((batch_feat_in[idx_in][:,:,:args.excit_dim], batch_mcep_rec[i-1]), 2), yz_in_, outpad_right=outpad_rights_eval[idx_in])
                                else:
                                    qy_logits[i], qz_alpha[i], h_z[i] \
                                        = model_encoder(torch.cat((batch_feat_in[idx_in][:,:,:args.excit_dim], batch_mcep_rec[i-1]), 2), outpad_right=outpad_rights_eval[idx_in])
                                i_1 = i-1
                                idx_in_1 = idx_in-1
                                feat_len = batch_mcep_rec[i_1].shape[1]
                                batch_mcep_rec[i_1] = batch_mcep_rec[i_1][:,outpad_lefts_eval[idx_in_1]:feat_len-outpad_rights_eval[idx_in_1]]
                                ...
                                if args.ar_enc:
                                    qy_logits[i], qz_alpha[i], h_z[i], yz_in[i] = model_encoder(batch_feat_in[idx_in], yz_in_, outpad_right=outpad_rights_eval[idx_in])
                                else:
                                    qy_logits[i], qz_alpha[i], h_z[i] = model_encoder(batch_feat_in[idx_in], outpad_right=outpad_rights_eval[idx_in])
                            idx_vq_z[i] = nn_search_batch(qz_alpha[i], model_vq.weight)
                            z[i] = model_vq(idx_vq_z[i])
                            idx_in += 1                            
                            if args.ar_dec:
                                batch_mcep_rec[i], h_mcep[i], x_in[i] = model_decoder(batch_sc_in[idx_in], z[i], x_in_, outpad_right=outpad_rights_eval[idx_in])
                                batch_mcep_cv[i_cv], h_mcep_cv[i_cv], x_in_cv[i_cv] = model_decoder(batch_sc_cv_in[i_cv], z[i], x_in_, outpad_right=outpad_rights_eval[idx_in])
                            else:
                                if args.diff:
                                    batch_mcep_rec[i], h_mcep[i], x_prev[i] = model_decoder(batch_sc_in[idx_in], z[i], x_prev=x_in_, outpad_right=outpad_rights_eval[idx_in])
                                    batch_mcep_cv[i_cv], h_mcep_cv[i_cv], x_prev_cv[i_cv] = model_decoder(batch_sc_cv_in[i_cv], z[i], x_prev=x_in_, outpad_right=outpad_rights_eval[idx_in])
                                else:
                            ...
                            if args.ar_enc:
                                qy_logits[j], qz_alpha[j], h_z[j], yz_in[j] \
                                    = model_encoder(torch.cat((batch_feat_cv_in[i_cv], batch_mcep_cv[i_cv]), 2), yz_in_, outpad_right=outpad_rights_eval[idx_in])
                            else:
                                qy_logits[j], qz_alpha[j], h_z[j] \
                                    = model_encoder(torch.cat((batch_feat_cv_in[i_cv], batch_mcep_cv[i_cv]), 2), outpad_right=outpad_rights_eval[idx_in])
                            feat_len = batch_mcep_rec[i].shape[1]
                            idx_in_1 = idx_in-1
                            batch_mcep_rec[i] = batch_mcep_rec[i][:,outpad_lefts_eval[idx_in_1]:feat_len-outpad_rights_eval[idx_in_1]]
                            batch_mcep_cv[i_cv] = batch_mcep_cv[i_cv][:,outpad_lefts_eval[idx_in_1]:feat_len-outpad_rights_eval[idx_in_1]]
                            ...
                            if n_half_cyc_eval > 1:
                                idx_in += 1
                                if args.ar_dec:
                                    batch_mcep_rec[j], h_mcep[j], x_in[j] = model_decoder(batch_sc_in[idx_in], z[j], x_in_, outpad_right=outpad_rights_eval[idx_in])
                                else:
                                    if args.diff:
                                        batch_mcep_rec[j], h_mcep[j], x_prev[j] = model_decoder(batch_sc_in[idx_in], z[j], x_prev=x_in_, outpad_right=outpad_rights_eval[idx_in])
                                    else:
                    ...
                    for i in range(n_half_cyc_eval):
                        ...
                        # elbo
                        batch_loss_elbo[i] = batch_loss_px_mcep_.sum() \
                                            + batch_loss_qy_py_ce + batch_loss_qy_py_prc + batch_loss_qz_pz_kl
                            ...
                            if n_half_cyc_eval > 1:
                                text_log += "[%ld] %.3f ; %.3f %.3f %.3f %% %.3f ; %.3f %.3f ; %.3f dB %.3f dB ;; " % (
                                    i+1, batch_loss_elbo[i].item(), batch_loss_px[i].item(), batch_loss_qy_py[i].item(), batch_loss_qy_py_err[i].item(), \
                                        batch_loss_qz_pz[i].item(), batch_loss_mcep_rec[i].item(), batch_loss_mcep_cv[i//2].item(), \
                                ...
                                text_log += "[%ld] %.3f ; %.3f %.3f %.3f %% %.3f , %.3f %.3f %% %.3f ; %.3f %.3f ; %.3f dB %.3f dB ;; " % (
                                    i+1, batch_loss_elbo[i].item(), batch_loss_px[i].item(), batch_loss_qy_py[i].item(), batch_loss_qy_py_err[i].item(), \
                                        ...
                                        batch_loss_qz_pz[i+1].item(), batch_loss_mcep_rec[i].item(), batch_loss_mcep_cv[i//2].item(), \
                                            batch_loss_powmcep[i].item(), batch_loss_mcep[i].item())
                        else:
                            text_log += "[%ld] %.3f ; %.3f %.3f %.3f %% %.3f ; %.3f ; %.3f dB %.3f dB ;; " % (
                                i+1, batch_loss_elbo[i].item(), batch_loss_px[i].item(), batch_loss_qy_py[i].item(), batch_loss_qy_py_err[i].item(), \
                                    batch_loss_qz_pz[i].item(), batch_loss_mcep_rec[i].item(), \
                                        batch_loss_powmcep[i].item(), batch_loss_mcep[i].item())
            ...
            if True:
                ...
                save_checkpoint(args.expdir, model_encoder, model_vq, model_decoder, optimizer, \
                    numpy_random_state, torch_random_state, epoch_idx + 1)
                    #optimizer_vq, numpy_random_state, torch_random_state, epoch_idx + 1)
            total = 0
            iter_count = 0
            for i in range(args.n_half_cyc):
                loss_elbo[i] = []
                ...
                for i in range(0,args.n_half_cyc,2):
                    ...
                    if i > 0:
                        ...
                        if args.detach:
                            if args.ar_enc:
                                qy_logits[i], qz_alpha[i], h_z[i], yz_in[i] \
                                    = model_encoder(torch.cat((batch_feat_in[idx_in][:,:,:args.excit_dim], batch_mcep_rec[i-1].detach()), 2), yz_in[i], h=h_z[i], outpad_right=outpad_rights[idx_in], do=True)
                            else:
                                qy_logits[i], qz_alpha[i], h_z[i] \
                            ...
                            if args.ar_enc:
                                qy_logits[i], qz_alpha[i], h_z[i], yz_in[i] \
                                    = model_encoder(torch.cat((batch_feat_in[idx_in][:,:,:args.excit_dim], batch_mcep_rec[i-1]), 2), yz_in[i], h=h_z[i], outpad_right=outpad_rights[idx_in], do=True)
                            else:
                                qy_logits[i], qz_alpha[i], h_z[i] \
                                    = model_encoder(torch.cat((batch_feat_in[idx_in][:,:,:args.excit_dim], batch_mcep_rec[i-1]), 2), h=h_z[i], outpad_right=outpad_rights[idx_in], do=True)
                        i_1 = i-1
                        idx_in_1 = idx_in-1
                        feat_len = batch_mcep_rec[i_1].shape[1]
                        ...
                        if args.ar_enc:
                            qy_logits[i], qz_alpha[i], h_z[i], yz_in[i] = model_encoder(batch_feat_in[idx_in], yz_in[i], h=h_z[i], outpad_right=outpad_rights[idx_in], do=True)
                        else:
                    ...
                    idx_in += 1
                    if args.ar_dec:
                        batch_mcep_rec[i], h_mcep[i], x_in[i] = model_decoder(batch_sc_in[idx_in], z_, x_in[i], h=h_mcep[i], outpad_right=outpad_rights[idx_in], do=True)
                        batch_mcep_cv[i_cv], h_mcep_cv[i_cv], x_in_cv[i_cv] = model_decoder(batch_sc_cv_in[i_cv], z_, x_in_cv[i_cv], h=h_mcep_cv[i_cv], outpad_right=outpad_rights[idx_in], do=True)
                    else:
                        if args.diff:
                            batch_mcep_rec[i], h_mcep[i], x_prev[i] = model_decoder(batch_sc_in[idx_in], z_, x_prev=x_prev[i], h=h_mcep[i], outpad_right=outpad_rights[idx_in], do=True)
                            batch_mcep_cv[i_cv], h_mcep_cv[i_cv], x_prev_cv[i_cv] = model_decoder(batch_sc_cv_in[i_cv], z_, x_prev=x_prev_cv[i_cv], h=h_mcep_cv[i_cv], outpad_right=outpad_rights[idx_in], do=True)
                        else:
                            batch_mcep_rec[i], h_mcep[i] = model_decoder(batch_sc_in[idx_in], z_, h=h_mcep[i], outpad_right=outpad_rights[idx_in], do=True)
                            batch_mcep_cv[i_cv], h_mcep_cv[i_cv] = model_decoder(batch_sc_cv_in[i_cv], z_, h=h_mcep_cv[i_cv], outpad_right=outpad_rights[idx_in], do=True)
                    feat_len = qy_logits[i].shape[1]
                    idx_in_1 = idx_in-1
                    ...
                    if args.detach:
                        if args.ar_enc:
                            qy_logits[j], qz_alpha[j], h_z[j], yz_in[j] \
                                = model_encoder(torch.cat((batch_feat_cv_in[i_cv], batch_mcep_cv[i_cv].detach()), 2), yz_in[j], h=h_z[j], outpad_right=outpad_rights[idx_in], do=True)
                        else:
                            qy_logits[j], qz_alpha[j], h_z[j] \
                        ...
                        if args.ar_enc:
                            qy_logits[j], qz_alpha[j], h_z[j], yz_in[j] \
                                = model_encoder(torch.cat((batch_feat_cv_in[i_cv], batch_mcep_cv[i_cv]), 2), yz_in[j], h=h_z[j], outpad_right=outpad_rights[idx_in], do=True)
                        else:
                            qy_logits[j], qz_alpha[j], h_z[j] \
                                = model_encoder(torch.cat((batch_feat_cv_in[i_cv], batch_mcep_cv[i_cv]), 2), h=h_z[j], outpad_right=outpad_rights[idx_in], do=True)
                    feat_len = batch_mcep_rec[i].shape[1]
                    idx_in_1 = idx_in-1
                    ...
                    if args.n_half_cyc > 1:
                        idx_in += 1
                        if args.ar_dec:
                            batch_mcep_rec[j], h_mcep[j], x_in[j] = model_decoder(batch_sc_in[idx_in], (z[j] - qz_alpha[j]).detach() + qz_alpha[j], x_in[j], h=h_mcep[j], outpad_right=outpad_rights[idx_in], do=True)
                        else:
                            if args.diff:
                                batch_mcep_rec[j], h_mcep[j], x_prev[j] = model_decoder(batch_sc_in[idx_in], (z[j] - qz_alpha[j]).detach() + qz_alpha[j], \
                                                                                x_prev=x_prev[j], h=h_mcep[j], outpad_right=outpad_rights[idx_in], do=True)
                            else:
                                batch_mcep_rec[j], h_mcep[j] = model_decoder(batch_sc_in[idx_in], (z[j] - qz_alpha[j]).detach() + qz_alpha[j], h=h_mcep[j], outpad_right=outpad_rights[idx_in], do=True)
                        feat_len = qy_logits[j].shape[1]
                        idx_in_1 = idx_in-1
                ...
                for i in range(0,args.n_half_cyc,2):
                    ...
                    if i > 0:
                        ...
                        if args.detach:
                            if args.ar_enc:
                                qy_logits[i], qz_alpha[i], h_z[i], yz_in[i] \
                                    = model_encoder(torch.cat((batch_feat_in[idx_in][:,:,:args.excit_dim], batch_mcep_rec[i-1].detach()), 2), yz_in_, outpad_right=outpad_rights[idx_in], do=True)
                            else:
                                qy_logits[i], qz_alpha[i], h_z[i] \
                            ...
                            if args.ar_enc:
                                qy_logits[i], qz_alpha[i], h_z[i], yz_in[i] \
                                    = model_encoder(torch.cat((batch_feat_in[idx_in][:,:,:args.excit_dim], batch_mcep_rec[i-1]), 2), yz_in_, outpad_right=outpad_rights[idx_in], do=True)
                            else:
                                qy_logits[i], qz_alpha[i], h_z[i] \
                                    = model_encoder(torch.cat((batch_feat_in[idx_in][:,:,:args.excit_dim], batch_mcep_rec[i-1]), 2), outpad_right=outpad_rights[idx_in], do=True)
                        i_1 = i-1
                        idx_in_1 = idx_in-1
                        feat_len = batch_mcep_rec[i_1].shape[1]
                        batch_mcep_rec[i_1] = batch_mcep_rec[i_1][:,outpad_lefts[idx_in_1]:feat_len-outpad_rights[idx_in_1]]
                        ...
                        if args.ar_enc:
                            qy_logits[i], qz_alpha[i], h_z[i], yz_in[i] = model_encoder(batch_feat_in[idx_in], yz_in_, outpad_right=outpad_rights[idx_in], do=True)
                        else:
                    ...
                    idx_in += 1
                    if args.ar_dec:
                        batch_mcep_rec[i], h_mcep[i], x_in[i] = model_decoder(batch_sc_in[idx_in], z_, x_in_, outpad_right=outpad_rights[idx_in], do=True)
                        batch_mcep_cv[i_cv], h_mcep_cv[i_cv], x_in_cv[i_cv] = model_decoder(batch_sc_cv_in[i_cv], z_, x_in_, outpad_right=outpad_rights[idx_in], do=True)
                    else:
                        if args.diff:
                            batch_mcep_rec[i], h_mcep[i], x_prev[i] = model_decoder(batch_sc_in[idx_in], z_, x_prev=x_in_, outpad_right=outpad_rights[idx_in], do=True)
                            batch_mcep_cv[i_cv], h_mcep_cv[i_cv], x_prev_cv[i_cv] = model_decoder(batch_sc_cv_in[i_cv], z_, x_prev=x_in_, outpad_right=outpad_rights[idx_in], do=True)
                        else:
                            batch_mcep_rec[i], h_mcep[i] = model_decoder(batch_sc_in[idx_in], z_, outpad_right=outpad_rights[idx_in], do=True)
                            batch_mcep_cv[i_cv], h_mcep_cv[i_cv] = model_decoder(batch_sc_cv_in[i_cv], z_, outpad_right=outpad_rights[idx_in], do=True)
                    feat_len = qy_logits[i].shape[1]
                    idx_in_1 = idx_in-1
                    qy_logits[i] = qy_logits[i][:,outpad_lefts[idx_in_1]:feat_len-outpad_rights[idx_in_1]]
                    qz_alpha[i] = qz_alpha[i][:,outpad_lefts[idx_in_1]:feat_len-outpad_rights[idx_in_1]]
                    ...
                    if args.detach:
                        if args.ar_enc:
                            qy_logits[j], qz_alpha[j], h_z[j], yz_in[j] \
                                = model_encoder(torch.cat((batch_feat_cv_in[i_cv], batch_mcep_cv[i_cv].detach()), 2), yz_in_, outpad_right=outpad_rights[idx_in], do=True)
                        else:
                            qy_logits[j], qz_alpha[j], h_z[j] \
                        ...
                        if args.ar_enc:
                            qy_logits[j], qz_alpha[j], h_z[j], yz_in[j] \
                                = model_encoder(torch.cat((batch_feat_cv_in[i_cv], batch_mcep_cv[i_cv]), 2), yz_in_, outpad_right=outpad_rights[idx_in], do=True)
                        else:
                            qy_logits[j], qz_alpha[j], h_z[j] \
                                = model_encoder(torch.cat((batch_feat_cv_in[i_cv], batch_mcep_cv[i_cv]), 2), outpad_right=outpad_rights[idx_in], do=True)
                    feat_len = batch_mcep_rec[i].shape[1]
                    idx_in_1 = idx_in-1
                    batch_mcep_rec[i] = batch_mcep_rec[i][:,outpad_lefts[idx_in_1]:feat_len-outpad_rights[idx_in_1]]
                    batch_mcep_cv[i_cv] = batch_mcep_cv[i_cv][:,outpad_lefts[idx_in_1]:feat_len-outpad_rights[idx_in_1]]
                    ...
                    if args.n_half_cyc > 1:
                        idx_in += 1
                        if args.ar_dec:
                            batch_mcep_rec[j], h_mcep[j], x_in[j] = model_decoder(batch_sc_in[idx_in], (z[j] - qz_alpha[j]).detach() + qz_alpha[j], x_in_, outpad_right=outpad_rights[idx_in], do=True)
                        else:
                            if args.diff:
                                batch_mcep_rec[j], h_mcep[j], x_prev[j] = model_decoder(batch_sc_in[idx_in], (z[j] - qz_alpha[j]).detach() + qz_alpha[j], x_prev=x_in_, outpad_right=outpad_rights[idx_in], do=True)
                            else:
                                batch_mcep_rec[j], h_mcep[j] = model_decoder(batch_sc_in[idx_in], (z[j] - qz_alpha[j]).detach() + qz_alpha[j], outpad_right=outpad_rights[idx_in], do=True)
                        feat_len = qy_logits[j].shape[1]
                        idx_in_1 = idx_in-1
                        qy_logits[j] = qy_logits[j][:,outpad_lefts[idx_in_1]:feat_len-outpad_rights[idx_in_1]]
                        qz_alpha[j] = qz_alpha[j][:,outpad_lefts[idx_in_1]:feat_len-outpad_rights[idx_in_1]]
            ...
            if len(idx_select) > 0:
                ...
                for j in range(len(idx_select)):
                    ...
                    for i in range(args.n_half_cyc):
                        ...
                        if i % 2 == 0:
                            batch_loss += torch.mean(criterion_ce(qy_logits_select_, batch_sc_)) \
                                                        + torch.mean(100*torch.sum(criterion_l1(F.softmax(qy_logits_select_, dim=-1), F.one_hot(batch_sc_, num_classes=n_spk).float()), -1))
                        else:
                            sc_cv_select_ = batch_sc_cv[i//2][k,:flens_utt]
                            batch_loss += torch.mean(criterion_ce(qy_logits_select_, sc_cv_select_)) \
                                                        + torch.mean(100*torch.sum(criterion_l1(F.softmax(qy_logits_select_, dim=-1), F.one_hot(sc_cv_select_, num_classes=n_spk).float()), -1))
                ...
                if len(idx_select_full) > 0:
                    ...
                    #    save_checkpoint(args.expdir, model_encoder, model_vq, model_decoder, optimizer_enc, \
                    #        optimizer_vq, optimizer_dec, np.random.get_state(), torch.get_rng_state(), iter_idx)
                    iter_count += 1
    ...
    torch.save({"model_encoder": model_encoder.state_dict(),
                "model_vq": model_vq.state_dict(),
                "model_decoder": model_decoder.state_dict()}, args.expdir + "/checkpoint-final.pkl")
