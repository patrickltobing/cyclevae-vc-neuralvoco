...
def save_checkpoint(checkpoint_dir, model_encoder, model_decoder,
...
def main():
    ...
    parser.add_argument("--f0in", default=True,
                        type=strtobool, help="use excitation features (f0+code-ap) for encoder input")
    ...
    if not args.f0in:
        model_encoder.scale_in.weight = torch.nn.Parameter(torch.unsqueeze(torch.diag(1.0/scale_stats[args.excit_dim:].data),2))
        model_encoder.scale_in.bias = torch.nn.Parameter(-(mean_stats[args.excit_dim:].data/scale_stats[args.excit_dim:].data))
    else:
    ...
    iter_idx = 0
    min_idx = -1
    #min_eval_loss_mcd_src_trg = 6.451324
    #min_eval_loss_mcd_src_trg_std = 0.875878
    #iter_idx = 296107
    #min_idx = 48 #resume58
    ...
    while epoch_idx < args.epoch_count:
        ...
        if c_idx < 0: # summarize epoch
                ...
                with torch.no_grad():
                    ...
                    if f_ss > 0:
                        ...
                        for i in range(0,n_half_cyc_eval,2):
                            ...
                            if i > 0:
                                ...
                                if args.ar_enc:
                                    if not args.f0in:
                                        qy_logits[i], qz_alpha[i], z[i], h_z[i], yz_in[i] \
                                            = model_encoder(batch_mcep_rec[i-1], yz_in[i], h=h_z[i], outpad_right=outpad_rights_eval[idx_in], sampling=False)
                                    else:
                                        qy_logits[i], qz_alpha[i], z[i], h_z[i], yz_in[i] \
                                            = model_encoder(torch.cat((batch_feat_in[idx_in][:,:,:args.excit_dim], batch_mcep_rec[i-1]), 2), yz_in[i], h=h_z[i], outpad_right=outpad_rights_eval[idx_in], sampling=False)
                                else:
                                    if not args.f0in:
                                        qy_logits[i], qz_alpha[i], z[i], h_z[i] \
                                            = model_encoder(batch_mcep_rec[i-1], h=h_z[i], outpad_right=outpad_rights_eval[idx_in], sampling=False)
                                    else:
                                        qy_logits[i], qz_alpha[i], z[i], h_z[i] \
                                ...
                                if args.ar_enc:
                                    if not args.f0in:
                                        qy_logits[i], qz_alpha[i], z[i], h_z[i], yz_in[i] \
                                            = model_encoder(batch_feat_in[idx_in][:,:,args.excit_dim:], yz_in[i], h=h_z[i], outpad_right=outpad_rights_eval[idx_in], sampling=False)
                                    else:
                                        qy_logits[i], qz_alpha[i], z[i], h_z[i], yz_in[i] \
                                            = model_encoder(batch_feat_in[idx_in], yz_in[i], h=h_z[i], outpad_right=outpad_rights_eval[idx_in], sampling=False)
                                else:
                                    if not args.f0in:
                                        qy_logits[i], qz_alpha[i], z[i], h_z[i] = model_encoder(batch_feat_in[idx_in][:,:,args.excit_dim:], h=h_z[i], outpad_right=outpad_rights_eval[idx_in], sampling=False)
                                    else:
                            ...
                            if args.ar_enc:
                                if not args.f0in:
                                    qy_logits[j], qz_alpha[j], z[j], h_z[j], yz_in[j] \
                                        = model_encoder(batch_mcep_cv[i_cv], yz_in[j], h=h_z[j], outpad_right=outpad_rights_eval[idx_in], sampling=False)
                                else:
                                    qy_logits[j], qz_alpha[j], z[j], h_z[j], yz_in[j] \
                                        = model_encoder(torch.cat((batch_feat_cv_in[i_cv], batch_mcep_cv[i_cv]), 2), yz_in[j], h=h_z[j], outpad_right=outpad_rights_eval[idx_in], sampling=False)
                            else:
                                if not args.f0in:
                                    qy_logits[j], qz_alpha[j], z[j], h_z[j] \
                                        = model_encoder(batch_mcep_cv[i_cv], h=h_z[j], outpad_right=outpad_rights_eval[idx_in], sampling=False)
                                else:
                                    qy_logits[j], qz_alpha[j], z[j], h_z[j] \
                        ...
                        if args.ar_enc:
                            ...
                            if not args.f0in:
                                _, _, trj_lat_src, _, _ = model_encoder(batch_feat_data_full[:,:,args.excit_dim:], yz_in_, sampling=False)
                            else:
                                _, _, trj_lat_src, _, _ = model_encoder(batch_feat_data_full, yz_in_, sampling=False)
                        else:
                            if not args.f0in:
                                _, _, trj_lat_src, _ = model_encoder(batch_feat_data_full[:,:,args.excit_dim:], sampling=False)
                            else:
                        ...
                        if pair_flag:
                            ...
                            if args.ar_enc:
                                if not args.f0in:
                                    _, _, trj_lat_trg, _, _ = model_encoder(batch_feat_trg_data_in[:,:,args.excit_dim:], yz_in_, sampling=False)
                                else:
                                    _, _, trj_lat_trg, _, _ = model_encoder(batch_feat_trg_data_in, yz_in_, sampling=False)
                            else:
                                if not args.f0in:
                                    _, _, trj_lat_trg, _ = model_encoder(batch_feat_trg_data_in[:,:,args.excit_dim:], sampling=False)
                                else:
                        ...
                        for i in range(0,n_half_cyc_eval,2):
                            ...
                            if i > 0:
                                ...
                                if args.ar_enc:
                                    if not args.f0in:
                                        qy_logits[i], qz_alpha[i], z[i], h_z[i], yz_in[i] \
                                            = model_encoder(batch_mcep_rec[i-1], yz_in_, outpad_right=outpad_rights_eval[idx_in], sampling=False)
                                    else:
                                        qy_logits[i], qz_alpha[i], z[i], h_z[i], yz_in[i] \
                                            = model_encoder(torch.cat((batch_feat_in[idx_in][:,:,:args.excit_dim], batch_mcep_rec[i-1]), 2), yz_in_, outpad_right=outpad_rights_eval[idx_in], sampling=False)
                                else:
                                    if not args.f0in:
                                        qy_logits[i], qz_alpha[i], z[i], h_z[i] \
                                            = model_encoder(batch_mcep_rec[i-1], outpad_right=outpad_rights_eval[idx_in], sampling=False)
                                    else:
                                        qy_logits[i], qz_alpha[i], z[i], h_z[i] \
                                ...
                                if args.ar_enc:
                                    if not args.f0in:
                                        qy_logits[i], qz_alpha[i], z[i], h_z[i], yz_in[i] = model_encoder(batch_feat_in[idx_in][:,:,args.excit_dim:], yz_in_, outpad_right=outpad_rights_eval[idx_in], sampling=False)
                                    else:
                                        qy_logits[i], qz_alpha[i], z[i], h_z[i], yz_in[i] = model_encoder(batch_feat_in[idx_in], yz_in_, outpad_right=outpad_rights_eval[idx_in], sampling=False)
                                else:
                                    if not args.f0in:
                                        qy_logits[i], qz_alpha[i], z[i], h_z[i] = model_encoder(batch_feat_in[idx_in][:,:,args.excit_dim:], outpad_right=outpad_rights_eval[idx_in], sampling=False)
                                    else:
                            ...
                            if args.ar_enc:
                                if not args.f0in:
                                    qy_logits[j], qz_alpha[j], z[j], h_z[j], yz_in[j] \
                                        = model_encoder(batch_mcep_cv[i_cv], yz_in_, outpad_right=outpad_rights_eval[idx_in], sampling=False)
                                else:
                                    qy_logits[j], qz_alpha[j], z[j], h_z[j], yz_in[j] \
                                        = model_encoder(torch.cat((batch_feat_cv_in[i_cv], batch_mcep_cv[i_cv]), 2), yz_in_, outpad_right=outpad_rights_eval[idx_in], sampling=False)
                            else:
                                if not args.f0in:
                                    qy_logits[j], qz_alpha[j], z[j], h_z[j] \
                                        = model_encoder(batch_mcep_cv[i_cv], outpad_right=outpad_rights_eval[idx_in], sampling=False)
                                else:
                                    qy_logits[j], qz_alpha[j], z[j], h_z[j] \
            ...
            if True:
                ...
                save_checkpoint(args.expdir, model_encoder, model_decoder, \
                    optimizer, numpy_random_state, torch_random_state, epoch_idx + 1)
            total = 0
            iter_count = 0
            for i in range(args.n_half_cyc):
                loss_elbo[i] = []
        ...
        if epoch_idx < args.epoch_count:
            ...
            if f_ss > 0:
                ...
                for i in range(0,args.n_half_cyc,2):
                    ...
                    if i > 0:
                        ...
                        if args.detach:
                            if args.ar_enc:
                                if not args.f0in:
                                    qy_logits[i], qz_alpha[i], h_z[i], yz_in[i] \
                                        = model_encoder(batch_mcep_rec[i-1].detach(), yz_in[i], h=h_z[i], outpad_right=outpad_rights[idx_in], do=True)
                                else:
                                    qy_logits[i], qz_alpha[i], h_z[i], yz_in[i] \
                                ...
                                if not args.f0in:
                                    qy_logits[i], qz_alpha[i], h_z[i] \
                                        = model_encoder(batch_mcep_rec[i-1].detach(), h=h_z[i], outpad_right=outpad_rights[idx_in], do=True)
                                else:
                                    qy_logits[i], qz_alpha[i], h_z[i] \
                            ...
                            if args.ar_enc:
                                if not args.f0in:
                                    qy_logits[i], qz_alpha[i], h_z[i], yz_in[i] \
                                        = model_encoder(batch_mcep_rec[i-1], yz_in[i], h=h_z[i], outpad_right=outpad_rights[idx_in], do=True)
                                else:
                                    qy_logits[i], qz_alpha[i], h_z[i], yz_in[i] \
                                ...
                                if not args.f0in:
                                    qy_logits[i], qz_alpha[i], h_z[i] \
                                        = model_encoder(batch_mcep_rec[i-1], h=h_z[i], outpad_right=outpad_rights[idx_in], do=True)
                                else:
                                    qy_logits[i], qz_alpha[i], h_z[i] \
                        ...
                        if args.ar_enc:
                            if not args.f0in:
                                qy_logits[i], qz_alpha[i], h_z[i], yz_in[i] = model_encoder(batch_feat_in[idx_in][:,:,args.excit_dim:], yz_in[i], h=h_z[i], outpad_right=outpad_rights[idx_in], do=True)
                            else:
                            ...
                            if not args.f0in:
                                qy_logits[i], qz_alpha[i], h_z[i] = model_encoder(batch_feat_in[idx_in][:,:,args.excit_dim:], h=h_z[i], outpad_right=outpad_rights[idx_in], do=True)
                            else:
                    ...
                    if not args.ar_dec:
                        ...
                        batch_mcep_rec[i], h_mcep[i], x_in[i] = model_decoder(batch_sc_in[idx_in], z[i], x_in[i], h=h_mcep[i], outpad_right=outpad_rights[idx_in], do=True)
                        batch_mcep_cv[i_cv], h_mcep_cv[i_cv], x_in_cv[i_cv] = model_decoder(batch_sc_cv_in[i_cv], z[i], x_in_cv[i_cv], h=h_mcep_cv[i_cv], outpad_right=outpad_rights[idx_in], do=True)
                    feat_len = qy_logits[i].shape[1]
                    idx_in_1 = idx_in-1
                    ...
                    if args.detach:
                        if args.ar_enc:
                            if not args.f0in:
                                qy_logits[j], qz_alpha[j], h_z[j], yz_in[j] \
                                    = model_encoder(batch_mcep_cv[i_cv].detach(), yz_in[j], h=h_z[j], outpad_right=outpad_rights[idx_in], do=True)
                            else:
                                qy_logits[j], qz_alpha[j], h_z[j], yz_in[j] \
                            ...
                            if not args.f0in:
                                qy_logits[j], qz_alpha[j], h_z[j] \
                                    = model_encoder(batch_mcep_cv[i_cv].detach(), h=h_z[j], outpad_right=outpad_rights[idx_in], do=True)
                            else:
                                qy_logits[j], qz_alpha[j], h_z[j] \
                        ...
                        if args.ar_enc:
                            if not args.f0in:
                                qy_logits[j], qz_alpha[j], h_z[j], yz_in[j] \
                                    = model_encoder(batch_mcep_cv[i_cv], yz_in[j], h=h_z[j], outpad_right=outpad_rights[idx_in], do=True)
                            else:
                                qy_logits[j], qz_alpha[j], h_z[j], yz_in[j] \
                            ...
                            if not args.f0in:
                                qy_logits[j], qz_alpha[j], h_z[j] \
                                    = model_encoder(batch_mcep_cv[i_cv], h=h_z[j], outpad_right=outpad_rights[idx_in], do=True)
                            else:
                                qy_logits[j], qz_alpha[j], h_z[j] \
                    ...
                    if args.n_half_cyc > 1:
                        ...
                        if not args.ar_dec:
                            ...
                            batch_mcep_rec[j], h_mcep[j], x_in[j] = model_decoder(batch_sc_in[idx_in], z[j], x_in[j], h=h_mcep[j], outpad_right=outpad_rights[idx_in], do=True)
                        feat_len = qy_logits[j].shape[1]
                        idx_in_1 = idx_in-1
                ...
                for i in range(0,args.n_half_cyc,2):
                    ...
                    if i > 0:
                        ...
                        if args.detach:
                            if args.ar_enc:
                                if not args.f0in:
                                    qy_logits[i], qz_alpha[i], h_z[i], yz_in[i] \
                                        = model_encoder(batch_mcep_rec[i-1].detach(), yz_in_, outpad_right=outpad_rights[idx_in], do=True)
                                else:
                                    qy_logits[i], qz_alpha[i], h_z[i], yz_in[i] \
                                ...
                                if not args.f0in:
                                    qy_logits[i], qz_alpha[i], h_z[i] \
                                        = model_encoder(batch_mcep_rec[i-1].detach(), outpad_right=outpad_rights[idx_in], do=True)
                                else:
                                    qy_logits[i], qz_alpha[i], h_z[i] \
                            ...
                            if args.ar_enc:
                                if not args.f0in:
                                    qy_logits[i], qz_alpha[i], h_z[i], yz_in[i] \
                                        = model_encoder(batch_mcep_rec[i-1], yz_in_, outpad_right=outpad_rights[idx_in], do=True)
                                else:
                                    qy_logits[i], qz_alpha[i], h_z[i], yz_in[i] \
                                ...
                                if not args.f0in:
                                    qy_logits[i], qz_alpha[i], h_z[i] \
                                        = model_encoder(batch_mcep_rec[i-1], outpad_right=outpad_rights[idx_in], do=True)
                                else:
                                    qy_logits[i], qz_alpha[i], h_z[i] \
                        ...
                        if args.ar_enc:
                            if not args.f0in:
                                qy_logits[i], qz_alpha[i], h_z[i], yz_in[i] = model_encoder(batch_feat_in[idx_in][:,:,args.excit_dim:], yz_in_, outpad_right=outpad_rights[idx_in], do=True)
                            else:
                                qy_logits[i], qz_alpha[i], h_z[i], yz_in[i] = model_encoder(batch_feat_in[idx_in], yz_in_, outpad_right=outpad_rights[idx_in], do=True)
                        else:
                            if not args.f0in:
                                qy_logits[i], qz_alpha[i], h_z[i] = model_encoder(batch_feat_in[idx_in][:,:,args.excit_dim:], outpad_right=outpad_rights[idx_in], do=True)
                            else:
                    ...
                    if not args.ar_dec:
                        ...
                        batch_mcep_rec[i], h_mcep[i], x_in[i] = model_decoder(batch_sc_in[idx_in], z[i], x_in_, outpad_right=outpad_rights[idx_in], do=True)
                        batch_mcep_cv[i_cv], h_mcep_cv[i_cv], x_in_cv[i_cv] = model_decoder(batch_sc_cv_in[i_cv], z[i], x_in_, outpad_right=outpad_rights[idx_in], do=True)
                    feat_len = qy_logits[i].shape[1]
                    idx_in_1 = idx_in-1
                    qy_logits[i] = qy_logits[i][:,outpad_lefts[idx_in_1]:feat_len-outpad_rights[idx_in_1]]
                    qz_alpha[i] = qz_alpha[i][:,outpad_lefts[idx_in_1]:feat_len-outpad_rights[idx_in_1]]
                    ...
                    if args.detach:
                        if args.ar_enc:
                            if not args.f0in:
                                qy_logits[j], qz_alpha[j], h_z[j], yz_in[j] \
                                    = model_encoder(batch_mcep_cv[i_cv].detach(), yz_in_, outpad_right=outpad_rights[idx_in], do=True)
                            else:
                                qy_logits[j], qz_alpha[j], h_z[j], yz_in[j] \
                            ...
                            if not args.f0in:
                                qy_logits[j], qz_alpha[j], h_z[j] \
                                    = model_encoder(batch_mcep_cv[i_cv].detach(), outpad_right=outpad_rights[idx_in], do=True)
                            else:
                                qy_logits[j], qz_alpha[j], h_z[j] \
                        ...
                        if args.ar_enc:
                            if not args.f0in:
                                qy_logits[j], qz_alpha[j], h_z[j], yz_in[j] \
                                    = model_encoder(batch_mcep_cv[i_cv], yz_in_, outpad_right=outpad_rights[idx_in], do=True)
                            else:
                                qy_logits[j], qz_alpha[j], h_z[j], yz_in[j] \
                            ...
                            if not args.f0in:
                                qy_logits[j], qz_alpha[j], h_z[j] \
                                    = model_encoder(batch_mcep_cv[i_cv], outpad_right=outpad_rights[idx_in], do=True)
                            else:
                                qy_logits[j], qz_alpha[j], h_z[j] \
                    ...
                    if args.n_half_cyc > 1:
                        ...
                        if not args.ar_dec:
                            ...
                            batch_mcep_rec[j], h_mcep[j], x_in[j] = model_decoder(batch_sc_in[idx_in], z[j], x_in_, outpad_right=outpad_rights[idx_in], do=True)
                        feat_len = qy_logits[j].shape[1]
                        idx_in_1 = idx_in-1
                        qy_logits[j] = qy_logits[j][:,outpad_lefts[idx_in_1]:feat_len-outpad_rights[idx_in_1]]
                        qz_alpha[j] = qz_alpha[j][:,outpad_lefts[idx_in_1]:feat_len-outpad_rights[idx_in_1]]
                ...
                for j in range(len(idx_select)):
                    ...
                    for i in range(args.n_half_cyc):
                        ...
                        if i % 2 == 0:
                            ...
                            batch_loss_qy_py_err_select += torch.mean(100*torch.sum(criterion_l1(F.softmax(qy_logits_select_, dim=-1), F.one_hot(batch_sc_, num_classes=n_spk).float()), -1))
                        else:
                            ...
                            batch_loss_qy_py_err_select += torch.mean(100*torch.sum(criterion_l1(F.softmax(qy_logits_select_, dim=-1), F.one_hot(sc_cv_select_, num_classes=n_spk).float()), -1))
            ...
            for i in range(args.n_half_cyc):
                ...
                # elbo
                batch_loss_elbo[i] = batch_loss_px_mcep_.sum() + batch_loss_qy_py_ce + batch_loss_qy_py_prc + batch_loss_qz_pz_kl
