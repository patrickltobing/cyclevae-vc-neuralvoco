...
def data_generator(dataloader, device, batch_size, upsampling_factor, limit_count=None, batch_sizes=None, n_bands=10, wlat_flag=False):
    ...
    while True:
        ...
        for idx, batch in enumerate(dataloader):
            ...
            feat_magsp_org = batch['feat_magsp_org'][:,:max_flen].to(device)
            feat_org = batch['feat_org'][:,:max_flen].to(device)
            ...
            while True:
                ...
                if len(del_index_utt) > 0:
                    ...
                    feat_magsp_org = torch.FloatTensor(np.delete(feat_magsp_org.cpu().data.numpy(), del_index_utt, axis=0)).to(device)
                    feat_org = torch.FloatTensor(np.delete(feat_org.cpu().data.numpy(), del_index_utt, axis=0)).to(device)
                ...
                if wlat_flag:
                    yield xs_c, xs_f, feat, feat_magsp_org, feat_org, lat, c_idx, idx, featfiles, x_bs, f_bs, x_ss, f_ss, n_batch_utt, del_index_utt, max_slen, \
...
def main():
    ...
    melfb_t = torch.FloatTensor(np.linalg.pinv(librosa.filters.mel(args.fs, args.fftl, n_mels=args.mcep_dim)).T)
    ...
    dataset = FeatureDatasetNeuVoco(wav_list, feat_list, pad_wav_transform, pad_feat_transform, args.upsampling_factor, 
                    args.string_path, wav_transform=wav_transform, n_bands=args.n_bands, with_excit=with_excit, cf_dim=args.cf_dim, spcidx=True,
                        pad_left=model_waveform.pad_left, pad_right=model_waveform.pad_right, string_path_ft=args.string_path_ft, wlat_flag=args.wlat_flag, worg_flag=True)
    ...
    while True:
        start = time.time()
        if args.wlat_flag:
            batch_x_c, batch_x_f, batch_feat, batch_feat_magsp_org, batch_feat_org, batch_lat, c_idx, utt_idx, featfile, x_bs, f_bs, x_ss, f_ss, n_batch_utt, \
        ...
        if c_idx < 0: # summarize epoch
            ...
            text_log = "(EPOCH:%d) average optimization loss = %.6f (+- %.6f) , %.6f (+- %.6f) %.6f (+- %.6f) , %.6f (+- %.6f) %.6f (+- %.6f) ; "\
                    ...
                    np.mean(loss_magsp), np.std(loss_magsp), np.mean(loss_magsp_dB), np.std(loss_magsp_dB),
            ...
            for i in range(args.n_bands):
                text_log += " [%d] %.6f (+- %.6f) %.6f (+- %.6f) %% %.6f (+- %.6f) %.6f (+- %.6f) %%" % (i+1,
                ...
                with torch.no_grad():
                    ...
                    if x_ss > 0:
                        if x_es <= max_slen:
                            ...
                            batch_feat_magsp_org = batch_feat_magsp_org[:,f_ss:f_es]
                            batch_feat_org = batch_feat_org[:,f_ss:f_es]
                        else:
                            ...
                            batch_feat_magsp_org = batch_feat_magsp_org[:,f_ss:]
                            batch_feat_org = batch_feat_org[:,f_ss:]
                    else:
                        ...
                        batch_feat_magsp_org = batch_feat_magsp_org[:,:f_es]
                        batch_feat_org = batch_feat_org[:,:f_es]
                    ...
                    batch_magsp_res = torch.matmul(batch_res_rest, melfb_t)
                    ...
                    if len(idx_select) > 0:
                        ...
                        if batch_loss_ms_err_magsp_select_count > 0:
                            ...
                            loss_ms_err_magsp.append(batch_loss_ms_err_magsp)
                    ...
                    batch_loss_magsp_ = torch.mean(torch.sum(criterion_l1(batch_magsp_res, batch_feat_magsp_org), -1), -1) \
                                        ...
                                        + torch.sqrt(torch.mean(torch.sum(criterion_l2(batch_magsp_res, batch_feat_magsp_org), -1), -1)) \
                                            + torch.sqrt(torch.mean(torch.mean(criterion_l2(batch_magsp_res, batch_feat_magsp_org), -1), -1))
                    batch_loss_magsp = batch_loss_magsp_.mean()
                    batch_loss_magsp_dB = torch.mean(torch.mean(torch.sqrt(torch.mean((20*(torch.log10(torch.clamp(batch_magsp_res, min=1e-16))
                                                            -torch.log10(torch.clamp(batch_feat_magsp_org, min=1e-16))))**2, -1)), -1))

                    batch_loss_ms_norm_, batch_loss_ms_err_ = criterion_ms(batch_res_rest, batch_feat_org_rest)
                    batch_loss_ms_norm = batch_loss_ms_norm_.mean()
                    batch_loss_ms_err = batch_loss_ms_err_.mean()
                    flag_ms_norm = False
                    if not torch.isinf(batch_loss_ms_norm) and not torch.isnan(batch_loss_ms_norm):
                        loss_ms_norm.append(batch_loss_ms_norm.item())
                        flag_ms_norm = True
                    flag_ms_err = False
                    if not torch.isinf(batch_loss_ms_err) and not torch.isnan(batch_loss_ms_err):
            ...
            eval_loss_ms_err_std = np.std(loss_ms_err)
            eval_loss_ms_norm_magsp = np.mean(loss_ms_norm_magsp)
            eval_loss_ms_norm_magsp_std = np.std(loss_ms_norm_magsp)
            eval_loss_ms_err_magsp = np.mean(loss_ms_err_magsp)
            eval_loss_ms_err_magsp_std = np.std(loss_ms_err_magsp)
            eval_loss_melsp = np.mean(loss_melsp)
            eval_loss_melsp_std = np.std(loss_melsp)
            eval_loss_melsp_dB = np.mean(loss_melsp_dB)
            eval_loss_melsp_dB_std = np.std(loss_melsp_dB)
            eval_loss_magsp = np.mean(loss_magsp)
            eval_loss_magsp_std = np.std(loss_magsp)
            eval_loss_magsp_dB = np.mean(loss_magsp_dB)
            eval_loss_magsp_dB_std = np.std(loss_magsp_dB)
            for i in range(args.n_bands):
            ...
            text_log = "(EPOCH:%d) average evaluation loss = %.6f (+- %.6f) , %.6f (+- %.6f) %.6f (+- %.6f) , %.6f (+- %.6f) %.6f (+- %.6f) ; "\
                    ...
                    eval_loss_magsp, eval_loss_magsp_std, eval_loss_magsp_dB, eval_loss_magsp_dB_std,
            ...
            if ((round(eval_loss_err_avg+eval_loss_err_avg_std,1)-0.4) <= round(min_eval_loss_err_avg+min_eval_loss_err_avg_std,1)) and \
                ...
                min_eval_loss_ms_err = eval_loss_ms_err
                min_eval_loss_ms_err_std = eval_loss_ms_err_std
                min_eval_loss_ms_norm_magsp = eval_loss_ms_norm_magsp
                min_eval_loss_ms_norm_magsp_std = eval_loss_ms_norm_magsp_std
                min_eval_loss_ms_err_magsp = eval_loss_ms_err_magsp
                min_eval_loss_ms_err_magsp_std = eval_loss_ms_err_magsp_std
                min_eval_loss_melsp = eval_loss_melsp
                min_eval_loss_melsp_std = eval_loss_melsp_std
                min_eval_loss_melsp_dB = eval_loss_melsp_dB
                min_eval_loss_melsp_dB_std = eval_loss_melsp_dB_std
                min_eval_loss_magsp = eval_loss_magsp
                min_eval_loss_magsp_std = eval_loss_magsp_std
                min_eval_loss_magsp_dB = eval_loss_magsp_dB
                min_eval_loss_magsp_dB_std = eval_loss_magsp_dB_std
                for i in range(args.n_bands):
            ...
            if change_min_flag:
                text_log = "min_eval_loss = %.6f (+- %.6f) , %.6f (+- %.6f) %.6f (+- %.6f) , %.6f (+- %.6f) %.6f (+- %.6f) ; "\
                        ...
                        min_eval_loss_magsp, min_eval_loss_magsp_std, min_eval_loss_magsp_dB, min_eval_loss_magsp_dB_std,
                        min_eval_loss_ce_avg, min_eval_loss_ce_avg_std, min_eval_loss_err_avg, min_eval_loss_err_avg_std,
        ...
        batch_loss_magsp_ = torch.mean(torch.sum(criterion_l1(batch_magsp_res, batch_feat_magsp_org), -1), -1) \
                            ...
                            + torch.sqrt(torch.mean(torch.sum(criterion_l2(batch_magsp_res, batch_feat_magsp_org), -1), -1)) \
                                + torch.sqrt(torch.mean(torch.mean(criterion_l2(batch_magsp_res, batch_feat_magsp_org), -1), -1))
        if iter_idx >= 50:
            batch_loss += batch_loss_magsp_.sum()
        batch_loss_magsp = batch_loss_magsp_.mean()
        batch_loss_magsp_dB = torch.mean(torch.mean(torch.sqrt(torch.mean((20*(torch.log10(torch.clamp(batch_magsp_res, min=1e-16))
                                                -torch.log10(torch.clamp(batch_feat_magsp_org, min=1e-16))))**2, -1)), -1))
        ...
        flag_ms_norm = False
        if not torch.isinf(batch_loss_ms_norm) and not torch.isnan(batch_loss_ms_norm):
            batch_loss += batch_loss_ms_norm_.sum()
            loss_ms_norm.append(batch_loss_ms_norm.item())
            flag_ms_norm = True
        flag_ms_err = False
        if not torch.isinf(batch_loss_ms_err) and not torch.isnan(batch_loss_ms_err):
            batch_loss += batch_loss_ms_err_.sum()
            loss_ms_err.append(batch_loss_ms_err.item())
            flag_ms_err = True
        ...
        if not torch.isinf(batch_loss_ms_norm_magsp) and not torch.isnan(batch_loss_ms_norm_magsp):
            if iter_idx >= 50:
                batch_loss += batch_loss_ms_magsp_norm_.sum()
            loss_ms_norm_magsp.append(batch_loss_ms_norm_magsp.item())
            flag_ms_norm_magsp = True
        flag_ms_err_magsp = False
        if not torch.isinf(batch_loss_ms_err_magsp) and not torch.isnan(batch_loss_ms_err_magsp):
            if iter_idx >= 50:
                batch_loss += batch_loss_ms_magsp_err_.sum()
            loss_ms_err_magsp.append(batch_loss_ms_err_magsp.item())
            flag_ms_err_magsp = True
