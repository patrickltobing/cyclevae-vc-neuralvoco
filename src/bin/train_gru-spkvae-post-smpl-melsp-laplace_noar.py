...
from vcneuvoco import GRU_POST_NET, GRU_SPK
from vcneuvoco import kl_laplace, ModulationSpectrumLoss, LaplaceLoss, kl_categorical_categorical_logits
...
def save_checkpoint(checkpoint_dir, model_encoder, model_spk, model_decoder, model_post, model_classifier,
...
def main():
    ...
    while epoch_idx < args.epoch_count:
        ...
        if c_idx < 0: # summarize epoch
            ...
            text_log += "%.6f (+- %.6f) %.6f (+- %.6f) %.6f (+- %.6f) %.6f (+- %.6f) ; %.6f (+- %.6f) , %.6f (+- %.6f) %.6f (+- %.6f) %.6f (+- %.6f) , %.6f (+- %.6f) %.6f (+- %.6f) %.6f (+- %.6f) ; "\
                "%.6f (+- %.6f) %.6f (+- %.6f) dB , %.6f (+- %.6f) %.6f (+- %.6f) dB , %.6f (+- %.6f) %.6f (+- %.6f) dB , %.6f (+- %.6f) %.6f (+- %.6f) dB ; "\
                "%.6f (+- %.6f) %.6f (+- %.6f) , %.6f (+- %.6f) %.6f (+- %.6f) , %.6f (+- %.6f) %.6f (+- %.6f) , %.6f (+- %.6f) %.6f (+- %.6f) ;; " % (
                np.mean(loss_elbo), np.std(loss_elbo), np.mean(loss_pxz_post), np.std(loss_pxz_post),
                np.mean(loss_qzx_pz), np.std(loss_qzx_pz), np.mean(loss_qyx_py), np.std(loss_qyx_py),
            ...
            while True:
                with torch.no_grad():
                    ...
                    for i in range(2):
                        if i == 0:
                            batch_sc_in[i] = F.pad(batch_sc.unsqueeze(1).float(), (first_pad_left-enc_pad_left,first_pad_right-enc_pad_right), "replicate").squeeze(1).long()
                        else:
                            batch_sc_in[i] = F.pad(batch_sc.unsqueeze(1).float(), (first_pad_left-enc_pad_left-dec_pad_left,first_pad_right-enc_pad_right-dec_pad_right), "replicate").squeeze(1).long()

                    batch_x_in_sc, _ = model_classifier(feat=batch_melsp)
                    batch_magsp_x_in_sc, _ = model_classifier(feat_aux=batch_magsp)
                    ## lat. infer.
                    idx_in = 0
                    batch_qyx, batch_qzx, batch_z, _ = model_encoder(batch_x_in, sampling=False)
                    feat_len = batch_qzx.shape[1]
                    batch_qyx = batch_qyx[:,outpad_lefts[idx_in]:feat_len-outpad_rights[idx_in]]
                    batch_qzx = batch_qzx[:,outpad_lefts[idx_in]:feat_len-outpad_rights[idx_in]]
                    ## reconst.
                    idx_in += 1
                    idx_in_1 = idx_in-1
                    batch_spk, _ = model_spk(batch_sc_in[idx_in_1], z=batch_z)
                    batch_x, _ = model_decoder(batch_z, y=batch_sc_in[idx_in_1], aux=batch_spk)
                    ...
                    batch_pxz_post, batch_x_post, _ = model_post(batch_x, y=batch_sc_in[idx_in_1], aux=batch_spk)
                    ...
                    for k in range(n_batch_utt):
                        ...
                        # lat/melsp/magsp cls
                        batch_loss_sc_z_ = torch.mean(kl_categorical_categorical_logits(p_spk, logits_p_spk, batch_z_sc[k,:flens_utt]), -1)
                        batch_sc_ = batch_sc[k,:flens_utt]
                        batch_loss_qyx_py_ = torch.mean(criterion_ce(batch_qyx[k,:flens_utt], batch_sc_))
                        ...
                        if k > 0:
                            ...
                            batch_loss_qyx_py += batch_loss_qyx_py_
                            ...
                            batch_loss_sc += 100*batch_loss_sc_z_ + batch_loss_qyx_py_ \
                    ...
                    total_eval_loss["eval/loss_qyx_py"].append(batch_loss_qyx_py.item())
                    ...
                    loss_qyx_py.append(batch_loss_qyx_py.item())
                    ...
                    text_log += "%.3f %.3f %.3f %.3f ; %.3f , %.3f %.3f %.3f , %.3f %.3f %.3f ; "\
                        "%.3f %.3f dB , %.3f %.3f dB , %.3f %.3f dB , %.3f %.3f dB ; "\
                        "%.3f %.3f , %.3f %.3f , %.3f %.3f , %.3f %.3f ;; " % (
                                batch_loss_elbo.item(), batch_loss_pxz_post.item(),
                                batch_loss_qzx_pz.item(), batch_loss_qyx_py.item(),
            ...
            eval_loss_qyx_py = np.mean(loss_qyx_py)
            eval_loss_qyx_py_std = np.std(loss_qyx_py)
            ...
            text_log += "%.6f (+- %.6f) %.6f (+- %.6f) %.6f (+- %.6f) %.6f (+- %.6f) ; %.6f (+- %.6f) , %.6f (+- %.6f) %.6f (+- %.6f) %.6f (+- %.6f) , %.6f (+- %.6f) %.6f (+- %.6f) %.6f (+- %.6f) ; "\
                ...
                "%.6f (+- %.6f) %.6f (+- %.6f) , %.6f (+- %.6f) %.6f (+- %.6f) , %.6f (+- %.6f) %.6f (+- %.6f) , %.6f (+- %.6f) %.6f (+- %.6f) ; "\
                "%.6f %.6f ;; " % (
                eval_loss_elbo, eval_loss_elbo_std, eval_loss_pxz_post, eval_loss_pxz_post_std,
                eval_loss_qzx_pz, eval_loss_qzx_pz_std, eval_loss_qyx_py, eval_loss_qyx_py_std,
            ...
            or round(eval_loss_magsp_x_post_dB,2) <= round(min_eval_loss_magsp_x_post_dB,2) \
                ...
                min_eval_loss_qyx_py = eval_loss_qyx_py
                min_eval_loss_qyx_py_std = eval_loss_qyx_py_std
            ...
            if change_min_flag:
                ...
                text_log += "%.6f (+- %.6f) %.6f (+- %.6f) %.6f (+- %.6f) %.6f (+- %.6f) ; %.6f (+- %.6f) , %.6f (+- %.6f) %.6f (+- %.6f) %.6f (+- %.6f) , %.6f (+- %.6f) %.6f (+- %.6f) %.6f (+- %.6f) ; "\
                    ...
                    "%.6f (+- %.6f) %.6f (+- %.6f) , %.6f (+- %.6f) %.6f (+- %.6f) , %.6f (+- %.6f) %.6f (+- %.6f) , %.6f (+- %.6f) %.6f (+- %.6f) ; "\
                    "%.6f %.6f ;; " % (
                    min_eval_loss_elbo, min_eval_loss_elbo_std, min_eval_loss_pxz_post, min_eval_loss_pxz_post_std,
                    min_eval_loss_qzx_pz, min_eval_loss_qzx_pz_std, min_eval_loss_qyx_py, min_eval_loss_qyx_py_std,
            ...
            if True:
                ...
                save_checkpoint(args.expdir, model_encoder, model_spk, model_decoder, model_post, model_classifier,
                    min_eval_loss_melsp_x_post_dB, min_eval_loss_melsp_x_post_dB_std,
                    min_eval_loss_magsp_x_post_dB, min_eval_loss_magsp_x_post_dB_std,
        ...
        if epoch_idx < args.epoch_count:
            ...
            for i in range(2):
                ...
                if f_ss_first_pad_left >= 0 and f_es_first_pad_right <= max_flen: # pad left and right available
                    ...
                    batch_sc_in[i] = F.pad(batch_sc[:,f_ss_first_pad_left:max_flen].unsqueeze(1).float(), (0,f_es_first_pad_right-max_flen), "replicate").squeeze(1).long()
                else: # pad left and right need additional replicate
                    batch_sc_in[i] = F.pad(batch_sc[:,:max_flen].unsqueeze(1).float(), (-f_ss_first_pad_left,f_es_first_pad_right-max_flen), "replicate").squeeze(1).long()
            ...
            if f_ss > 0:
                if len(del_index_utt) > 0:
                    ...
                    h_z = torch.FloatTensor(np.delete(h_z.cpu().data.numpy(),
                                                    del_index_utt, axis=1)).to(device)
                    h_spk = torch.FloatTensor(np.delete(h_spk.cpu().data.numpy(),
                                                    del_index_utt, axis=1)).to(device)
                ...
                ## lat. infer.
                idx_in = 0
                batch_qyx, batch_qzx, batch_z, h_z = model_encoder(batch_x_in, outpad_right=outpad_rights[idx_in], h=h_z, do=True)
                feat_len = batch_qzx.shape[1]
                batch_qyx = batch_qyx[:,outpad_lefts[idx_in]:feat_len-outpad_rights[idx_in]]
                batch_qzx = batch_qzx[:,outpad_lefts[idx_in]:feat_len-outpad_rights[idx_in]]
                ## reconst.
                idx_in += 1
                idx_in_1 = idx_in-1
                batch_spk, h_spk = model_spk(batch_sc_in[idx_in_1], z=batch_z, outpad_right=outpad_rights[idx_in], h=h_spk, do=True)
                batch_x, h_x = model_decoder(batch_z, y=batch_sc_in[idx_in_1], aux=batch_spk, outpad_right=outpad_rights[idx_in], h=h_x, do=True)
                batch_z = batch_z[:,outpad_lefts[idx_in_1]:batch_z.shape[1]-outpad_rights[idx_in_1]]
                ...
                if dec_pad_right > 0:
                    ...
                    batch_spk = batch_spk[:,dec_pad_left:]
                batch_pxz_post, batch_x_post, h_x_post = model_post(batch_x, y=batch_sc_in[idx_in_1], aux=batch_spk, outpad_right=outpad_rights[idx_in], h=h_x_post, do=True)
                batch_x = batch_x[:,outpad_lefts[idx_in_1]:batch_x.shape[1]-outpad_rights[idx_in_1]]
                batch_magsp_x = torch.matmul((torch.exp(batch_x)-1)/10000, melfb_t)
                batch_magsp_x_post = torch.matmul((torch.exp(batch_x_post)-1)/10000, melfb_t)
                ...
                ## lat. infer.
                idx_in = 0
                batch_qyx, batch_qzx, batch_z, h_z = model_encoder(batch_x_in, outpad_right=outpad_rights[idx_in], do=True)
                feat_len = batch_qzx.shape[1]
                batch_qyx = batch_qyx[:,outpad_lefts[idx_in]:feat_len-outpad_rights[idx_in]]
                batch_qzx = batch_qzx[:,outpad_lefts[idx_in]:feat_len-outpad_rights[idx_in]]
                ## reconst.
                idx_in += 1
                idx_in_1 = idx_in-1
                batch_spk, h_spk = model_spk(batch_sc_in[idx_in_1], z=batch_z, outpad_right=outpad_rights[idx_in], do=True)
                batch_x, h_x = model_decoder(batch_z, y=batch_sc_in[idx_in_1], aux=batch_spk, outpad_right=outpad_rights[idx_in], do=True)
                batch_z = batch_z[:,outpad_lefts[idx_in_1]:batch_z.shape[1]-outpad_rights[idx_in_1]]
                ...
                if dec_pad_right > 0:
                    ...
                    batch_spk = batch_spk[:,dec_pad_left:]
                batch_pxz_post, batch_x_post, h_x_post = model_post(batch_x, y=batch_sc_in[idx_in_1], aux=batch_spk, outpad_right=outpad_rights[idx_in], do=True)
                batch_x = batch_x[:,outpad_lefts[idx_in_1]:batch_x.shape[1]-outpad_rights[idx_in_1]]
                batch_magsp_x = torch.matmul((torch.exp(batch_x)-1)/10000, melfb_t)
                batch_magsp_x_post = torch.matmul((torch.exp(batch_x_post)-1)/10000, melfb_t)
            ...
            if len(idx_select) > 0:
                ...
                for j in range(len(idx_select)):
                    ...
                    batch_sc_ = batch_sc[k,:flens_utt]
                    batch_loss_sc_select += 100*torch.mean(kl_categorical_categorical_logits(p_spk, logits_p_spk, batch_z_sc[k,:flens_utt])) \
                                            + torch.mean(criterion_ce(batch_qyx[k,:flens_utt], batch_sc_)) \
                ...
                if len(idx_select_full) > 0:
                    ...
                    batch_qyx = torch.index_select(batch_qyx,0,idx_select_full)
            ...
            # lat/melsp/magsp cls
            batch_loss_sc_z_ = torch.mean(kl_categorical_categorical_logits(p_spk, logits_p_spk, batch_z_sc), -1)
            batch_loss_sc_z = batch_loss_sc_z_.mean()

            batch_sc_ = batch_sc.reshape(-1)

            batch_loss_qyx_py_ = torch.mean(criterion_ce(batch_qyx.reshape(-1, n_spk), batch_sc_).reshape(n_batch_utt, -1), -1)
            batch_loss_qyx_py = batch_loss_qyx_py_.mean()
            ...
            batch_loss_sc = 100*batch_loss_sc_z_.sum() + batch_loss_qyx_py_.sum() \
            ...
            total_train_loss["train/loss_qyx_py"].append(batch_loss_qyx_py.item())
            loss_elbo.append(batch_loss_elbo.item())
            loss_pxz_post.append(batch_loss_pxz_post.item())
            loss_qzx_pz.append(batch_loss_qzx_pz.item())
            loss_qyx_py.append(batch_loss_qyx_py.item())
            ...
            text_log += "%.3f %.3f %.3f %.3f ; %.3f , %.3f %.3f %.3f , %.3f %.3f %.3f ; "\
                ...
                "%.3f %.3f , %.3f %.3f , %.3f %.3f , %.3f %.3f ;; " % (
                        batch_loss_elbo.item(), batch_loss_pxz_post.item(),
                        batch_loss_qzx_pz.item(), batch_loss_qyx_py.item(),
                        batch_loss_sc_z.item(), batch_loss_sc_x_in.item(), batch_loss_sc_x.item(), batch_loss_sc_x_post.item(),
                        batch_loss_sc_magsp_x_in.item(), batch_loss_sc_magsp_x.item(), batch_loss_sc_magsp_x_post.item(),
                        batch_loss_melsp_x.item(), batch_loss_melsp_x_dB.item(), batch_loss_melsp_x_post.item(), batch_loss_melsp_x_post_dB.item(),
                        batch_loss_magsp_x.item(), batch_loss_magsp_x_dB.item(), batch_loss_magsp_x_post.item(), batch_loss_magsp_x_post_dB.item(),
                        batch_loss_ms_norm_x.item(), batch_loss_ms_err_x.item(), batch_loss_ms_norm_x_post.item(), batch_loss_ms_err_x_post.item(),
