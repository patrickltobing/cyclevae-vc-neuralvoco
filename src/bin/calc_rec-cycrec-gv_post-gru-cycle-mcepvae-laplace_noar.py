            ...
            for feat_file in feat_list:
                ...
                with torch.no_grad():
                    ...
                    if outpad_rights[2] > 0:
                        feat_rec = torch.cat((feat_excit, cvmcep_src_post[:,outpad_lefts[2]:-outpad_rights[2]]), 2)[0].cpu().data.numpy()
                    else:
                        feat_rec = torch.cat((feat_excit, cvmcep_src_post[:,outpad_lefts[2]:]), 2)[0].cpu().data.numpy()
                    feat_cyc = torch.cat((feat_excit, cvmcep_cyc_post), 2)[0].cpu().data.numpy()

                    if outpad_rights[2] > 0:
                        cvmcep_src = np.array(cvmcep_src_post[:,outpad_lefts[2]:-outpad_rights[2]][0].cpu().data.numpy(), dtype=np.float64)
                    else:
                        cvmcep_src = np.array(cvmcep_src_post[:,outpad_lefts[2]:][0].cpu().data.numpy(), dtype=np.float64)
