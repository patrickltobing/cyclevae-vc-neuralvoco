...
from vcneuvoco import GRU_VAE_ENCODER_, GRU_SPEC_DECODER_, GRU_LAT_FEAT_CLASSIFIER
from vcneuvoco import GRU_POST_NET_
...
def save_checkpoint(checkpoint_dir, model_encoder, model_decoder, model_post,
        model_classifier, min_eval_loss_melsp_x_dB, min_eval_loss_melsp_x_dB_std,
    ...
    checkpoint = {
        ...
        "min_eval_loss_melsp_x_dB": min_eval_loss_melsp_x_dB,
        "min_eval_loss_melsp_x_dB_std": min_eval_loss_melsp_x_dB_std,
...
def main():
    ...
    # define network
    model_encoder = GRU_VAE_ENCODER_(
        in_dim=args.mel_dim,
    ...
    model_decoder = GRU_SPEC_DECODER_(
    ...
    min_eval_loss_melsp_x_dB = 99999999.99
    min_eval_loss_melsp_x_dB_std = 99999999.99
    ...
    if args.resume is not None:
        ...
        min_eval_loss_melsp_x_dB = checkpoint["min_eval_loss_melsp_x_dB"]
        min_eval_loss_melsp_x_dB_std = checkpoint["min_eval_loss_melsp_x_dB_std"]
    ...
    while epoch_idx < args.epoch_count:
        ...
        if c_idx < 0: # summarize epoch
                ...
                with torch.no_grad():
                    ...
                    for k in range(n_batch_utt):
                        ...
                        batch_loss_melsp_x_ = torch.mean(torch.sum(criterion_l1(melsp_x, melsp), -1)) \
                                                + torch.mean(torch.sum(criterion_l2(melsp_x, melsp), -1)) \
                                            + torch.mean(torch.mean(criterion_l1(melsp_x, melsp), -1)) \
                                                + torch.mean(torch.mean(criterion_l2(melsp_x, melsp), -1))
                        ...
                        batch_loss_melsp_x_post_ = torch.mean(torch.sum(criterion_l1(melsp_x_post, melsp), -1)) \
                                                        + torch.mean(torch.sum(criterion_l2(melsp_x_post, melsp), -1)) \
                                                    + torch.mean(torch.mean(criterion_l1(melsp_x_post, melsp), -1)) \
                                                        + torch.mean(torch.mean(criterion_l2(melsp_x_post, melsp), -1))
                        ...
                        batch_loss_magsp_x_ = torch.mean(torch.sum(criterion_l1(magsp_x, magsp), -1)) \
                                                + torch.mean(torch.sum(criterion_l2(magsp_x, magsp), -1)) \
                                            + torch.mean(torch.mean(criterion_l1(magsp_x, magsp), -1)) \
                                                + torch.mean(torch.mean(criterion_l2(magsp_x, magsp), -1))
                        ...
                        batch_loss_magsp_x_post_ = torch.mean(torch.sum(criterion_l1(magsp_x_post, magsp), -1)) \
                                                        + torch.mean(torch.sum(criterion_l2(magsp_x_post, magsp), -1)) \
                                                    + torch.mean(torch.mean(criterion_l1(magsp_x_post, magsp), -1)) \
                                                        + torch.mean(torch.mean(criterion_l2(magsp_x_post, magsp), -1))
                        ...
                        if not torch.isinf(batch_loss_ms_norm_x_) and not torch.isnan(batch_loss_ms_norm_x_) and batch_loss_ms_norm_x_ <= 3:
                            batch_loss_p += batch_loss_ms_norm_x_
                        if not torch.isinf(batch_loss_ms_err_x_) and not torch.isnan(batch_loss_ms_err_x_) and batch_loss_ms_err_x_ <= 3:
                        ...
                        if not torch.isinf(batch_loss_ms_norm_x_post_) and not torch.isnan(batch_loss_ms_norm_x_post_) and batch_loss_ms_norm_x_post_ <= 3:
                            batch_loss_p += batch_loss_ms_norm_x_post_
                        if not torch.isinf(batch_loss_ms_err_x_post_) and not torch.isnan(batch_loss_ms_err_x_post_) and batch_loss_ms_err_x_post_ <= 3:
                            batch_loss_p += batch_loss_ms_err_x_post_
                        batch_loss_ms_norm_magsp_x_, batch_loss_ms_err_magsp_x_ = criterion_ms(magsp_x, magsp)
                        if not torch.isinf(batch_loss_ms_norm_magsp_x_) and not torch.isnan(batch_loss_ms_norm_magsp_x_) and batch_loss_ms_norm_magsp_x_ <= 3:
                            batch_loss_p += batch_loss_ms_norm_magsp_x_
                        if not torch.isinf(batch_loss_ms_err_magsp_x_) and not torch.isnan(batch_loss_ms_err_magsp_x_) and batch_loss_ms_err_magsp_x_ <= 3:
                        ...
                        if not torch.isinf(batch_loss_ms_norm_magsp_x_post_) and not torch.isnan(batch_loss_ms_norm_magsp_x_post_) and batch_loss_ms_norm_magsp_x_post_ <= 3:
                            batch_loss_p += batch_loss_ms_norm_magsp_x_post_
                        if not torch.isinf(batch_loss_ms_err_magsp_x_post_) and not torch.isnan(batch_loss_ms_err_magsp_x_post_) and batch_loss_ms_err_magsp_x_post_ <= 3:
                        ...
                        if k > 0:
                            ...
                            batch_loss_sc += batch_loss_sc_z_ \
                                            + batch_loss_sc_x_in_ + batch_loss_sc_x_ + batch_loss_sc_x_post \
                                                + batch_loss_sc_magsp_x_in_ + batch_loss_sc_magsp_x_ + batch_loss_sc_magsp_x_post
                        else:
                            batch_loss_sc_z = batch_loss_sc_z_
                            batch_loss_sc_x_in = batch_loss_sc_x_in_
                            batch_loss_sc_x = batch_loss_sc_x_
            ...
            logging.info("%s (%.3f min., %.3f sec / batch)" % (text_log, total / 60.0, total / iter_count))
            if eval_loss_melsp_x_dB <= min_eval_loss_melsp_x_dB and eval_loss_melsp_x_post_dB <= min_eval_loss_melsp_x_post_dB:
            ...
            if True:
                ...
                save_checkpoint(args.expdir, model_encoder, model_decoder, model_post,
                    model_classifier, min_eval_loss_melsp_x_dB, min_eval_loss_melsp_x_dB_std,
                    min_eval_loss_melsp_x_post_dB, min_eval_loss_melsp_x_post_dB_std,
        ...
        if epoch_idx < args.epoch_count:
            ...
            if len(idx_select) > 0:
                ...
                for j in range(len(idx_select)):
                    ...
                    batch_loss_p_select += criterion_laplace(batch_pxz_post_[:,:args.mel_dim], batch_pxz_post_[:,args.mel_dim:], melsp).mean() \
                                            + torch.mean(torch.sum(criterion_l1(melsp_x, melsp), -1)) \
                                                + torch.mean(torch.sum(criterion_l2(melsp_x, melsp), -1)) \
                                            + torch.mean(torch.mean(criterion_l1(melsp_x, melsp), -1)) \
                                                + torch.mean(torch.mean(criterion_l2(melsp_x, melsp), -1)) \
            ...
            batch_loss_melsp_x_ = torch.mean(torch.sum(criterion_l1(batch_x, batch_melsp), -1), -1) \
                                    + torch.mean(torch.sum(criterion_l2(batch_x, batch_melsp), -1), -1) \
                                + torch.mean(torch.mean(criterion_l1(batch_x, batch_melsp), -1), -1) \
                                    + torch.mean(torch.mean(criterion_l2(batch_x, batch_melsp), -1), -1)
            ...
            batch_loss_melsp_x_post_ = torch.mean(torch.sum(criterion_l1(batch_x_post, batch_melsp), -1), -1) \
                                        + torch.mean(torch.sum(criterion_l2(batch_x_post, batch_melsp), -1), -1) \
                                    + torch.mean(torch.mean(criterion_l1(batch_x_post, batch_melsp), -1), -1) \
                                        + torch.mean(torch.mean(criterion_l2(batch_x_post, batch_melsp), -1), -1)
            ...
            batch_loss_magsp_x_ = torch.mean(torch.sum(criterion_l1(batch_magsp_x, batch_magsp), -1), -1) \
                                    + torch.mean(torch.sum(criterion_l2(batch_magsp_x, batch_magsp), -1), -1) \
                                + torch.mean(torch.mean(criterion_l1(batch_magsp_x, batch_magsp), -1), -1) \
                                    + torch.mean(torch.mean(criterion_l2(batch_magsp_x, batch_magsp), -1), -1)
            ...
            batch_loss_magsp_x_post_ = torch.mean(torch.sum(criterion_l1(batch_magsp_x_post, batch_magsp), -1), -1) \
                                        + torch.mean(torch.sum(criterion_l2(batch_magsp_x_post, batch_magsp), -1), -1) \
                                    + torch.mean(torch.mean(criterion_l1(batch_magsp_x_post, batch_magsp), -1), -1) \
                                        + torch.mean(torch.mean(criterion_l2(batch_magsp_x_post, batch_magsp), -1), -1)
            ...
            if not torch.isinf(batch_loss_ms_norm_x_post) and not torch.isnan(batch_loss_ms_norm_x_post) and batch_loss_ms_norm_x_post <= 3:
                batch_loss_p += batch_loss_ms_norm_x_post_.sum()
            batch_loss_ms_err_x_post = batch_loss_ms_err_x_post_.mean()
            if not torch.isinf(batch_loss_ms_err_x_post) and not torch.isnan(batch_loss_ms_err_x_post) and batch_loss_ms_err_x_post <= 3:
            ...
            if not torch.isinf(batch_loss_ms_norm_magsp_x) and not torch.isnan(batch_loss_ms_norm_magsp_x) and batch_loss_ms_norm_magsp_x <= 3:
                batch_loss_p += batch_loss_ms_norm_magsp_x_.sum()
            batch_loss_ms_err_magsp_x = batch_loss_ms_err_magsp_x_.mean()
            if not torch.isinf(batch_loss_ms_err_magsp_x) and not torch.isnan(batch_loss_ms_err_magsp_x) and batch_loss_ms_err_magsp_x <= 3:
            ...
            if not torch.isinf(batch_loss_ms_norm_magsp_x_post) and not torch.isnan(batch_loss_ms_norm_magsp_x_post) and batch_loss_ms_norm_magsp_x_post <= 3:
                batch_loss_p += batch_loss_ms_norm_magsp_x_post_.sum()
            batch_loss_ms_err_magsp_x_post = batch_loss_ms_err_magsp_x_post_.mean()
            if not torch.isinf(batch_loss_ms_err_magsp_x_post) and not torch.isnan(batch_loss_ms_err_magsp_x_post) and batch_loss_ms_err_magsp_x_post <= 3:
