...
from vcneuvoco import LaplaceLoss, decode_mu_law_torch, MultiResolutionSTFTLoss
...
def save_checkpoint(checkpoint_dir, model_encoder_melsp, model_decoder_melsp_fix, model_decoder_melsp, model_encoder_excit,
        model_spk_fix, model_spk,
    ...
    checkpoint = {
        ...
        "model_spk_fix": model_spk_fix.state_dict(),
...
def main():
    ...
    parameters = filter(lambda p: p.requires_grad, model_spk_fix.parameters())
    parameters = sum([np.prod(p.size()) for p in parameters]) / 1000000
    logging.info('Trainable Parameters (spk_fix): %.3f million' % parameters)
    ...
    if args.resume is None:
        ...
        model_spk_fix.load_state_dict(checkpoint["model_spk"])
    ...
    for param in model_encoder_excit.parameters():
        param.requires_grad = False
    for param in model_spk_fix.parameters():
        param.requires_grad = False
    ...
    if args.resume is not None:
        ...
        model_decoder_melsp.load_state_dict(checkpoint["model_decoder_melsp"])
        model_encoder_excit.load_state_dict(checkpoint["model_encoder_excit"])
        model_spk_fix.load_state_dict(checkpoint["model_spk_fix"])
        model_spk.load_state_dict(checkpoint["model_spk"])
        if args.spkidtr_dim > 0:
            model_spkidtr.load_state_dict(checkpoint["model_spkidtr"])
        if model_waveform.use_weight_norm:
    ...
    while True:
        ...
        if c_idx < 0: # summarize epoch
            ...
            for i in range(args.n_half_cyc):
                if i % 2 == 0:
                    text_log += "[%ld] %.6f (+- %.6f) ; %.6f (+- %.6f) %.6f (+- %.6f) , %.6f (+- %.6f) %.6f (+- %.6f) %.6f (+- %.6f) dB , %.6f (+- %.6f) %.6f (+- %.6f) %.6f (+- %.6f) dB ; "\
                    ...
                    text_log += "[%ld] %.6f (+- %.6f) ; %.6f (+- %.6f) , %.6f (+- %.6f) %.6f (+- %.6f) dB , %.6f (+- %.6f) %.6f (+- %.6f) dB ; "\
                            "%.6f (+- %.6f) %.6f (+- %.6f) , %.6f (+- %.6f) %.6f (+- %.6f) ; "\
                            "%.6f (+- %.6f) %.6f (+- %.6f) %% %.6f (+- %.6f) %.6f (+- %.6f) %% %.6f (+- %.6f) %.6f (+- %.6f) %% , %.6f (+- %.6f) %.6f (+- %.6f) , %.6f (+- %.6f) %.6f (+- %.6f) ; " % (i+1,
                        np.mean(loss_px[i]), np.std(loss_px[i]),
            ...
            while True:
                with torch.no_grad():
                    ...
                    if f_ss > 0:
                        ...
                        for i in range(0,n_half_cyc_eval,2):
                            ...
                            if len(del_index_utt) > 0:
                                ...
                                h_spk_cv[i_cv] = torch.FloatTensor(np.delete(h_spk_cv[i_cv].cpu().data.numpy(),
                                                                del_index_utt, axis=1)).to(device)
                                h_spk_cv_fix = torch.FloatTensor(np.delete(h_spk_cv_fix.cpu().data.numpy(),
                                                                del_index_utt, axis=1)).to(device)
                                ...
                                h_melsp_cv[i_cv] = torch.FloatTensor(np.delete(h_melsp_cv[i_cv].cpu().data.numpy(),
                                                                del_index_utt, axis=1)).to(device)
                                h_melsp_cv_fix = torch.FloatTensor(np.delete(h_melsp_cv_fix.cpu().data.numpy(),
                                                                del_index_utt, axis=1)).to(device)
                            ...
                            if args.spkidtr_dim > 0:
                                ...
                                batch_spk_cv_fix, h_spk_cv_fix = model_spk_fix(spk_cv_code_in, z=z_cat, outpad_right=outpad_rights[idx_in], h=h_spk_cv_fix)
                            else:
                                ...
                                batch_spk_cv_fix, h_spk_cv_fix = model_spk_fix(batch_sc_cv_in[i_cv_in], z=z_cat, outpad_right=outpad_rights[idx_in], h=h_spk_cv_fix)
                            ...
                            if args.spkidtr_dim > 0:
                                ...
                                _, batch_melsp_cv_fix, h_melsp_cv_fix = model_decoder_melsp_fix(z_cat, y=spk_cv_code_in, aux=batch_spk_cv_fix,
                                                    outpad_right=outpad_rights[idx_in], h=h_melsp_cv_fix)
                            else:
                                ...
                                batch_pdf_cv[i_cv], batch_melsp_cv[i_cv], h_melsp_cv[i_cv] = model_decoder_melsp(z_cat, y=batch_sc_cv_in[i_cv_in], aux=batch_spk_cv,
                                                    outpad_right=outpad_rights[idx_in], h=h_melsp_cv[i_cv])
                                _, batch_melsp_cv_fix, h_melsp_cv_fix = model_decoder_melsp_fix(z_cat, y=batch_sc_cv_in[i_cv_in], aux=batch_spk_cv_fix,
                                                    outpad_right=outpad_rights[idx_in], h=h_melsp_cv_fix)
                            ...
                            if wav_pad_right > 0:
                                cv_feat = batch_melsp_cv_fix[:,wav_pad_left:-wav_pad_right]
                            else:
                                cv_feat = batch_melsp_cv_fix[:,wav_pad_left:]
                            idx_in_1 = idx_in-1
                        ...
                        for i in range(0,n_half_cyc_eval,2):
                            ...
                            if args.spkidtr_dim > 0:
                                ...
                                batch_spk_cv_fix, h_spk_cv_fix = model_spk_fix(spk_cv_code_in, z=z_cat, outpad_right=outpad_rights[idx_in])
                            else:
                                ...
                                batch_spk_cv_fix, h_spk_cv_fix = model_spk_fix(batch_sc_cv_in[i_cv_in], z=z_cat, outpad_right=outpad_rights[idx_in])
                            ## melsp reconstruction & conversion
                            idx_in += 1
                            i_cv_in += 1
                            if spk_pad_right > 0:
                    ...
                    text_log = "batch eval loss [%d] %d %d %d %d " % (c_idx+1, x_ss, x_bs, f_ss, f_bs)
                    for i in range(n_half_cyc_eval):
                        if i == 0:
                            text_log += "[%ld] %.3f ; %.3f %.3f , %.3f %.3f %.3f dB , %.3f %.3f %.3f dB ; %.3f %.3f , %.3f %.3f ; %.3f %.3f %% %.3f %.3f %% %.3f %.3f %% , %.3f %.3f , %.3f %.3f ; " % (i+1,
                            ...
                            text_log += "[%ld] %.3f ; %.3f , %.3f %.3f dB , %.3f %.3f dB ; %.3f %.3f , %.3f %.3f ; %.3f %.3f %% %.3f %.3f %% %.3f %.3f %% , %.3f %.3f , %.3f %.3f ; " % (i+1,
                                batch_loss_px[i].item(),
            ...
            for i in range(n_half_cyc_eval):
                if i % 2 == 0:
                    text_log += "[%ld] %.6f (+- %.6f) ; %.6f (+- %.6f) %.6f (+- %.6f) , %.6f (+- %.6f) %.6f (+- %.6f) %.6f (+- %.6f) dB , %.6f (+- %.6f) %.6f (+- %.6f) %.6f (+- %.6f) dB ; %.6f %.6f " % (i+1,
                    ...
                    text_log += "[%ld] %.6f (+- %.6f) ; %.6f (+- %.6f) , %.6f (+- %.6f) %.6f (+- %.6f) dB , %.6f (+- %.6f) %.6f (+- %.6f) dB ; " % (i+1,
                        eval_loss_px[i], eval_loss_px_std[i],
            ...
            if True:
                ...
                if model_waveform.use_weight_norm:
                    torch.nn.utils.remove_weight_norm(model_waveform.scale_in)
                save_checkpoint(args.expdir, model_encoder_melsp, model_decoder_melsp_fix, model_decoder_melsp, model_encoder_excit,
                    model_spk_fix, model_spk, model_waveform,
        ...
        if f_ss > 0:
            ...
            for i in range(0,args.n_half_cyc,2):
                ...
                batch_x_output_fb[i] = pqmf.synthesis(batch_x_output[i].transpose(1,2))[:,0]
                if wav_pad_right > 0:
                    cv_feat = batch_melsp_cv_fix[:,wav_pad_left:-wav_pad_right].detach()
                else:
                    cv_feat = batch_melsp_cv_fix[:,wav_pad_left:].detach()
                idx_in_1 = idx_in-1
                feat_len = batch_melsp_rec[i].shape[1]
                batch_pdf_rec[i] = batch_pdf_rec[i][:,outpad_lefts[idx_in_1]:feat_len-outpad_rights[idx_in_1]]
                batch_melsp_rec[i] = batch_melsp_rec[i][:,outpad_lefts[idx_in_1]:feat_len-outpad_rights[idx_in_1]]
        ...
        for i in range(args.n_half_cyc):
            ...
            # elbo
            batch_loss += batch_loss_px_sum + batch_loss_wave
        ...
        if flag:
            logging.info("explode grad %s" % (model_explode))
            optimizer.zero_grad()
            text_log = "batch loss [%d] %d %d %d %d " % (c_idx+1, x_ss, x_bs, f_ss, f_bs)
            for i in range(args.n_half_cyc):
                if i == 0:
                    text_log += "[%ld] %.3f ; %.3f %.3f , %.3f %.3f %.3f dB , %.3f %.3f %.3f dB ; %.3f %.3f , %.3f %.3f ; %.3f %.3f %% %.3f %.3f %% %.3f %.3f %% , %.3f %.3f , %.3f %.3f ; " % (i+1,
                        batch_loss_px[i].item(),
