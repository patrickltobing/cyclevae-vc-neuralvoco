...
from vcneuvoco import GRU_EXCIT_DECODER
from vcneuvoco import kl_laplace
...
def save_checkpoint(checkpoint_dir, model_encoder_mcep, model_decoder_mcep, model_encoder_excit, model_decoder_excit, \
...
def main():
    ...
    parser.add_argument("--ar_dec", default=True,
                        type=strtobool, help="output feedback (AR flow) decoder")
    parser.add_argument("--ar_f0", default=True,
    ...
    if args.ar_enc:
        yz_in_zeros_ = torch.zeros(1, 1, n_spk+args.lat_dim*2)
        yz_in_e_zeros_ = torch.zeros(1, 1, n_spk+args.lat_dim_e*2)
    if args.ar_dec:
        x_in_zeros_ = (torch.zeros(1, 1, args.mcep_dim)-mean_stats[args.excit_dim:])/scale_stats[args.excit_dim:]
    if args.ar_f0:
        e_in_zeros_ = torch.cat((torch.zeros(1,1,1), (torch.zeros(1,1,1)-mean_stats[1:2])/scale_stats[1:2], \
                        torch.zeros(1,1,1), (torch.zeros(1,1,args.cap_dim)-mean_stats[3:args.excit_dim])/scale_stats[3:args.excit_dim]), 2)
    ...
    if model_decoder_excit.use_weight_norm:
        ...
        if model_decoder_excit.cap_dim is not None:
    ...
    if args.cap_dim is not None:
        model_decoder_excit.scale_out_cap.weight = torch.nn.Parameter(torch.unsqueeze(torch.diag(scale_stats[3:args.excit_dim].data),2))
        model_decoder_excit.scale_out_cap.bias = torch.nn.Parameter(mean_stats[3:args.excit_dim].data)
    ...
    dataset = FeatureDatasetCycMceplf0WavVAE(feat_list, pad_feat_transform, spk_list, stats_list, \
                    args.n_half_cyc, args.string_path, excit_dim=args.excit_dim)
    ...
    dataset_eval = FeatureDatasetEvalCycMceplf0WavVAE(feat_list_eval_src_list, pad_feat_transform, spk_list, \
                    stats_list, args.string_path, excit_dim=args.excit_dim)
    ...
    iter_idx = 0
    min_idx = -1
    #min_eval_loss_mcep[0] = 4.964284
    #min_eval_loss_mcep_std[0] = 0.424672
    #min_eval_loss_mcep_cv[0] = 38.681251
    #min_eval_loss_mcep_rec[0] = 26.759531
    #min_eval_loss_mcd_src_trg = 6.044570
    #min_eval_loss_mcd_src_trg_std = 0.512962
    #iter_idx = 174478 #resume31
    #min_idx = 29
        ...
        if c_idx < 0: # summarize epoch
            ...
            for i in range(args.n_half_cyc):
                if i % 2 == 0:
                    ...
                    if args.cap_dim is not None:
                        ...
                        text_log += "[%ld] %.6f (+- %.6f) ; %.6f (+- %.6f) %.6f (+- %.6f) %.6f (+- %.6f) %% %.6f (+- %.6f) , %.6f (+- %.6f) %.6f (+- %.6f) %% %.6f (+- %.6f) ; "\
                            "%.6f (+- %.6f) ; %.6f (+- %.6f) dB %.6f (+- %.6f) dB , %.6f (+- %.6f) %% %.6f (+- %.6f) Hz ;; " % (i+1, \
                            np.mean(loss_elbo[i]), np.std(loss_elbo[i]),
                            np.mean(loss_px[i]), np.std(loss_px[i]), np.mean(loss_qy_py[i]), np.std(loss_qy_py[i]), np.mean(loss_qy_py_err[i]), np.std(loss_qy_py_err[i]), \
                            np.mean(loss_qz_pz[i]), np.std(loss_qz_pz[i]), np.mean(loss_qy_py_e[i]), np.std(loss_qy_py_e[i]), np.mean(loss_qy_py_err_e[i]), np.std(loss_qy_py_err_e[i]), \
                            np.mean(loss_qz_pz_e[i]), np.std(loss_qz_pz_e[i]), np.mean(loss_mcep_rec[i]), np.std(loss_mcep_rec[i]), \
            ...
            while True:
                with torch.no_grad():
                    ...
                    if f_ss > 0:
                        ...
                        for i in range(0,n_half_cyc_eval,2):
                            ...
                            if i > 0:
                                idx_in += 1
                                if args.ar_enc:
                                    qy_logits[i], qz_alpha[i], z[i], h_z[i], yz_in[i] \
                                        = model_encoder_mcep(torch.cat((batch_feat_in[idx_in][:,:,:args.excit_dim], batch_mcep_rec[i-1]), 2), \
                                                                yz_in=yz_in[i], h=h_z[i], outpad_right=outpad_rights_eval[idx_in], sampling=False)
                                    qy_logits_e[i], qz_alpha_e[i], z_e[i], h_z_e[i], yz_in_e[i] \
                                        = model_encoder_excit(torch.cat((batch_lf0_rec[i-1][:,:,:args.excit_dim], batch_feat_in[idx_in][:,:,args.excit_dim:]), 2), \
                                                                yz_in=yz_in_e[i], h=h_z_e[i], outpad_right=outpad_rights_eval[idx_in], sampling=False)
                                else:
                                    qy_logits[i], qz_alpha[i], z[i], h_z[i] \
                                        = model_encoder_mcep(torch.cat((batch_feat_in[idx_in][:,:,:args.excit_dim], batch_mcep_rec[i-1]), 2), \
                                                                h=h_z[i], outpad_right=outpad_rights_eval[idx_in], sampling=False)
                                    qy_logits_e[i], qz_alpha_e[i], z_e[i], h_z_e[i] \
                                        = model_encoder_excit(torch.cat((batch_lf0_rec[i-1][:,:,:args.excit_dim], batch_feat_in[idx_in][:,:,args.excit_dim:]), 2), \
                                                                h=h_z_e[i], outpad_right=outpad_rights_eval[idx_in], sampling=False)
                                i_1 = i-1
                                idx_in_1 = idx_in-1
                                ...
                                if args.ar_enc:
                                    qy_logits[i], qz_alpha[i], z[i], h_z[i], yz_in[i] \
                                            = model_encoder_mcep(batch_feat_in[idx_in], yz_in=yz_in[i], h=h_z[i], outpad_right=outpad_rights_eval[idx_in], sampling=False)
                                    qy_logits_e[i], qz_alpha_e[i], z_e[i], h_z_e[i], yz_in_e[i] \
                                            = model_encoder_excit(batch_feat_in[idx_in], yz_in=yz_in_e[i], h=h_z_e[i], outpad_right=outpad_rights_eval[idx_in], sampling=False)
                                else:
                                    qy_logits[i], qz_alpha[i], z[i], h_z[i] \
                                            = model_encoder_mcep(batch_feat_in[idx_in], h=h_z[i], outpad_right=outpad_rights_eval[idx_in], sampling=False)
                                    qy_logits_e[i], qz_alpha_e[i], z_e[i], h_z_e[i] \
                                            = model_encoder_excit(batch_feat_in[idx_in], h=h_z_e[i], outpad_right=outpad_rights_eval[idx_in], sampling=False)
                            ...
                            if args.ar_dec:
                                ...
                                batch_mcep_cv[i_cv], h_mcep_cv[i_cv], x_in_cv[i] = model_decoder_mcep(batch_sc_cv_in[i_cv], z[i], x_in_cv[i_cv], h=h_mcep_cv[i_cv], outpad_right=outpad_rights_eval[idx_in])
                            else:
                            ...
                            idx_in += 1
                            if args.ar_enc:
                                qy_logits[j], qz_alpha[j], z[j], h_z[j], yz_in[j] \
                                    = model_encoder_mcep(torch.cat((batch_feat_cv_in[i_cv], batch_mcep_cv[i_cv]), 2), yz_in=yz_in[j], h=h_z[j], outpad_right=outpad_rights_eval[idx_in], sampling=False)
                                qy_logits_e[j], qz_alpha_e[j], z_e[j], h_z_e[j], yz_in_e[j] \
                                    = model_encoder_excit(torch.cat((batch_lf0_cv[i_cv][:,:,:args.excit_dim], batch_mcep_cv[i_cv]), 2), yz_in=yz_in_e[j], h=h_z_e[j], outpad_right=outpad_rights_eval[idx_in], sampling=False)
                            else:
                                qy_logits[j], qz_alpha[j], z[j], h_z[j] \
                                    = model_encoder_mcep(torch.cat((batch_feat_cv_in[i_cv], batch_mcep_cv[i_cv]), 2), h=h_z[j], outpad_right=outpad_rights_eval[idx_in], sampling=False)
                                qy_logits_e[j], qz_alpha_e[j], z_e[j], h_z_e[j] \
                                    = model_encoder_excit(torch.cat((batch_lf0_cv[i_cv][:,:,:args.excit_dim], batch_mcep_cv[i_cv]), 2), h=h_z_e[j], outpad_right=outpad_rights_eval[idx_in], sampling=False)
                        ...
                        if args.ar_enc:
                            ...
                            _, _, trj_lat_src, _, _ = model_encoder_mcep(batch_feat_data_full, yz_in_, sampling=False)
                            _, _, trj_lat_src_e, _, _ = model_encoder_excit(batch_feat_data_full, yz_in_e_, sampling=False)
                        else:
                            _, _, trj_lat_src, _ = model_encoder_mcep(batch_feat_data_full, sampling=False)
                            _, _, trj_lat_src_e, _ = model_encoder_excit(batch_feat_data_full, sampling=False)
                        ...
                        if pair_flag:
                            ...
                            if args.ar_enc:
                                _, _, trj_lat_trg, _, _ = model_encoder_mcep(batch_feat_trg_data_in, yz_in_, sampling=False)
                                _, _, trj_lat_trg_e, _, _ = model_encoder_excit(batch_feat_trg_data_in, yz_in_e_, sampling=False)
                            else:
                                _, _, trj_lat_trg, _ = model_encoder_mcep(batch_feat_trg_data_in, sampling=False)
                                _, _, trj_lat_trg_e, _ = model_encoder_excit(batch_feat_trg_data_in, sampling=False)
                            ...
                            for k in range(n_batch_utt):
                                if src_trg_flag[k]:
                                    ...
                                    trj_lat_src_e_ = np.array(torch.index_select(trj_lat_src_e[k],0,spcidx_src[k,:flens_spc_src[k]]).cpu().data.numpy(), dtype=np.float64)
                                    ...
                                    trj_lat_trg_e_ = np.array(torch.index_select(trj_lat_trg_e[k],0,spcidx_src_trg[k,:flens_spc_src_trg[k]]).cpu().data.numpy(), dtype=np.float64)
                                    ...
                                    if args.cap_dim is not None:
                                        ...
                                        trj_trg_cap = -torch.exp(trj_trg_[:,3:args.excit_dim])
                                    ...
                                    if args.cap_dim is not None:
                                        ...
                                        logging.info('acc cv %s %s %.3f dB %.3f dB %.3f %% %.3f Hz %.3f %.3f' % (featfile[k], \
                                            spk_cv[k], batch_mcdpow_src_trg, batch_mcd_src_trg, \
                                                batch_uv_src_trg, batch_f0_src_trg, \
                        ...
                        for i in range(0,n_half_cyc_eval,2):
                            ...
                            if i > 0:
                                ...
                                if args.ar_enc:
                                    qy_logits[i], qz_alpha[i], z[i], h_z[i], yz_in[i] \
                                        = model_encoder_mcep(torch.cat((batch_feat_in[idx_in][:,:,:args.excit_dim], batch_mcep_rec[i-1]), 2), \
                                                                yz_in=yz_in_, outpad_right=outpad_rights_eval[idx_in], sampling=False)
                                    qy_logits_e[i], qz_alpha_e[i], z_e[i], h_z_e[i], yz_in_e[i] \
                                        = model_encoder_excit(torch.cat((batch_lf0_rec[i-1][:,:,:args.excit_dim], batch_feat_in[idx_in][:,:,args.excit_dim:]), 2), \
                                                                yz_in=yz_in_e_, outpad_right=outpad_rights_eval[idx_in], sampling=False)
                                else:
                                    qy_logits[i], qz_alpha[i], z[i], h_z[i] \
                                        = model_encoder_mcep(torch.cat((batch_feat_in[idx_in][:,:,:args.excit_dim], batch_mcep_rec[i-1]), 2), outpad_right=outpad_rights_eval[idx_in], sampling=False)
                                    qy_logits_e[i], qz_alpha_e[i], z_e[i], h_z_e[i] \
                                        = model_encoder_excit(torch.cat((batch_lf0_rec[i-1][:,:,:args.excit_dim], batch_feat_in[idx_in][:,:,args.excit_dim:]), 2), outpad_right=outpad_rights_eval[idx_in], sampling=False)
                                i_1 = i-1
                                idx_in_1 = idx_in-1
                                feat_len = batch_mcep_rec[i_1].shape[1]
                                batch_mcep_rec[i_1] = batch_mcep_rec[i_1][:,outpad_lefts_eval[idx_in_1]:feat_len-outpad_rights_eval[idx_in_1]]
                                ...
                                if args.ar_enc:
                                    qy_logits[i], qz_alpha[i], z[i], h_z[i], yz_in[i] \
                                            = model_encoder_mcep(batch_feat_in[idx_in], yz_in_, outpad_right=outpad_rights_eval[idx_in], sampling=False)
                                    qy_logits_e[i], qz_alpha_e[i], z_e[i], h_z_e[i], yz_in_e[i] \
                                            = model_encoder_excit(batch_feat_in[idx_in], yz_in_e_, outpad_right=outpad_rights_eval[idx_in], sampling=False)
                                else:
                                    qy_logits[i], qz_alpha[i], z[i], h_z[i] \
                                            = model_encoder_mcep(batch_feat_in[idx_in], outpad_right=outpad_rights_eval[idx_in], sampling=False)
                                    qy_logits_e[i], qz_alpha_e[i], z_e[i], h_z_e[i] \
                                            = model_encoder_excit(batch_feat_in[idx_in], outpad_right=outpad_rights_eval[idx_in], sampling=False)
                            ## reconstruction and conversion
                            idx_in += 1
                            if args.ar_dec:
                            ...
                            if args.ar_enc:
                                qy_logits[j], qz_alpha[j], z[j], h_z[j], yz_in[j] \
                                    = model_encoder_mcep(torch.cat((batch_feat_cv_in[i_cv], batch_mcep_cv[i_cv]), 2), yz_in=yz_in_, outpad_right=outpad_rights_eval[idx_in], sampling=False)
                                qy_logits_e[j], qz_alpha_e[j], z_e[j], h_z_e[j], yz_in_e[j] \
                                    = model_encoder_excit(torch.cat((batch_lf0_cv[i_cv][:,:,:args.excit_dim], batch_mcep_cv[i_cv]), 2), yz_in=yz_in_e_, outpad_right=outpad_rights_eval[idx_in], sampling=False)
                            else:
                                qy_logits[j], qz_alpha[j], z[j], h_z[j] \
                                    = model_encoder_mcep(torch.cat((batch_feat_cv_in[i_cv], batch_mcep_cv[i_cv]), 2), outpad_right=outpad_rights_eval[idx_in], sampling=False)
                                qy_logits_e[j], qz_alpha_e[j], z_e[j], h_z_e[j] \
                                    = model_encoder_excit(torch.cat((batch_lf0_cv[i_cv][:,:,:args.excit_dim], batch_mcep_cv[i_cv]), 2), outpad_right=outpad_rights_eval[idx_in], sampling=False)
                            feat_len = batch_mcep_rec[i].shape[1]
                            idx_in_1 = idx_in-1
                            batch_mcep_rec[i] = batch_mcep_rec[i][:,outpad_lefts_eval[idx_in_1]:feat_len-outpad_rights_eval[idx_in_1]]
                            batch_mcep_cv[i_cv] = batch_mcep_cv[i_cv][:,outpad_lefts_eval[idx_in_1]:feat_len-outpad_rights_eval[idx_in_1]]
                    ...
                    logging.info(torch.exp(batch_feat[i,:2,1]))
                    ...
                    logging.info(-torch.exp(batch_feat[i,:2,3:args.excit_dim]))
                    ...
                    # loss_compute
                    uv = batch_feat[:,:,0]
                    f0 = torch.exp(batch_feat[:,:,1])
                    if args.cap_dim is not None:
                        uvcap = batch_feat[:,:,2]
                        cap = -torch.exp(batch_feat[:,:,3:args.excit_dim])
                        ...
                        if i % 2 == 0:
                            if n_half_cyc_eval > 1:
                                if args.cap_dim is not None:
                                    ...
                                    text_log += "[%ld] %.3f ; %.3f %.3f %.3f %% %.3f , %.3f %.3f %% %.3f ; %.3f %.3f ; %.3f dB %.3f dB , %.3f %% %.3f %% , %.3f Hz %.3f Hz ;; " % (
                                        i+1, batch_loss_elbo[i].item(), batch_loss_px[i].item(), batch_loss_qy_py[i].item(), batch_loss_qy_py_err[i].item(), \
                                            batch_loss_qz_pz[i].item(), batch_loss_qy_py_e[i].item(), batch_loss_qy_py_err_e[i].item(), \
                                            batch_loss_qz_pz_e[i].item(), batch_loss_mcep_rec[i].item(), batch_loss_mcep_cv[i//2].item(), \
                                                batch_loss_powmcep[i].item(), batch_loss_mcep[i].item(), \
                                ...
                                if args.cap_dim is not None:
                                    ...
                                    text_log += "[%ld] %.3f ; %.3f %.3f %.3f %% %.3f , %.3f %.3f %% %.3f , %.3f %.3f %% %.3f , %.3f %.3f %% %.3f ; "\
                                        "%.3f %.3f ; %.3f dB %.3f dB , %.3f %% %.3f %% , %.3f Hz %.3f Hz ;; " % (
                                        i+1, batch_loss_elbo[i].item(), batch_loss_px[i].item(), batch_loss_qy_py[i].item(), batch_loss_qy_py_err[i].item(), \
                                            batch_loss_qz_pz[i].item(), batch_loss_qy_py[i+1].item(), batch_loss_qy_py_err[i+1].item(), \
                                            batch_loss_qz_pz[i+1].item(), batch_loss_qy_py_e[i].item(), batch_loss_qy_py_err_e[i].item(), \
                                            batch_loss_qz_pz_e[i].item(), batch_loss_qy_py_e[i+1].item(), batch_loss_qy_py_err_e[i+1].item(), \
                            ...
                            if args.cap_dim is not None:
                                ...
                                text_log += "[%ld] %.3f ; %.3f %.3f %.3f %% %.3f , %.3f %.3f %% %.3f ; %.3f ; %.3f dB %.3f dB , %.3f %% %.3f Hz ;; " % (
                                    i+1, batch_loss_elbo[i].item(), batch_loss_px[i].item(), batch_loss_qy_py[i].item(), batch_loss_qy_py_err[i].item(), \
                                        batch_loss_qz_pz[i].item(), batch_loss_qy_py_e[i].item(), batch_loss_qy_py_err_e[i].item(), \
                                        batch_loss_qz_pz_e[i].item(), batch_loss_mcep_rec[i].item(), \
                                            batch_loss_powmcep[i].item(), batch_loss_mcep[i].item(), \
                    ...
                    if n_half_cyc_eval > 1:
                        if args.cap_dim is not None:
                            ...
                            if pair_exist:
                                text_log += "[%ld] %.6f (+- %.6f) ; %.6f (+- %.6f) %.6f (+- %.6f) %.6f (+- %.6f) %% %.6f (+- %.6f) , %.6f (+- %.6f) %.6f (+- %.6f) %% %.6f (+- %.6f) ; " \
                                    "%.6f (+- %.6f) %.6f (+- %.6f) ; %.6f (+- %.6f) dB %.6f (+- %.6f) dB , %.6f (+- %.6f) %% " \
                                    "%.6f (+- %.6f) %% , %.6f (+- %.6f) Hz %.6f (+- %.6f) Hz ; %.6f %.6f " \
                                    "%.6f (+- %.6f) dB %.6f (+- %.6f) dB %.6f (+- %.6f) %% %.6f (+- %.6f) Hz %.6f (+- %.6f) %.6f (+- %.6f) ;; " % (i+1, \
                                    eval_loss_elbo[i], eval_loss_elbo_std[i], eval_loss_px[i], eval_loss_px_std[i], \
                                    eval_loss_qy_py[i], eval_loss_qy_py_std[i], eval_loss_qy_py_err[i], eval_loss_qy_py_err_std[i], \
                                    eval_loss_qz_pz[i], eval_loss_qz_pz_std[i], \
                                    eval_loss_qy_py_e[i], eval_loss_qy_py_e_std[i], eval_loss_qy_py_err_e[i], eval_loss_qy_py_err_e_std[i], \
                                    ...
                                    eval_loss_gv_src_src, eval_loss_gv_src_trg, eval_loss_mcdpow_src_trg, eval_loss_mcdpow_src_trg_std, \
                                    eval_loss_mcd_src_trg, eval_loss_mcd_src_trg_std, eval_loss_uv_src_trg, eval_loss_uv_src_trg_std, \
                                    eval_loss_f0_src_trg, eval_loss_f0_src_trg_std, eval_loss_lat_dist_rmse, eval_loss_lat_dist_rmse_std, \
                                    eval_loss_lat_dist_cossim, eval_loss_lat_dist_cossim_std)
                            else:
                                text_log += "[%ld] %.6f (+- %.6f) ; %.6f (+- %.6f) %.6f (+- %.6f) %.6f (+- %.6f) %% %.6f (+- %.6f) , %.6f (+- %.6f) %.6f (+- %.6f) %% %.6f (+- %.6f) ; " \
                                    "%.6f (+- %.6f) %.6f (+- %.6f) ; %.6f (+- %.6f) dB %.6f (+- %.6f) dB , %.6f (+- %.6f) %% " \
                                    "%.6f (+- %.6f) %% , %.6f (+- %.6f) Hz %.6f (+- %.6f) Hz ; %.6f %.6f " \
                                    "n/a (+- n/a) dB n/a (+- n/a) dB n/a (+- n/a) %% n/a (+- n/a) Hz n/a (+- n/a) n/a (+- n/a) ;; " % (i+1, \
                                    eval_loss_elbo[i], eval_loss_elbo_std[i], eval_loss_px[i], eval_loss_px_std[i], \
                                    eval_loss_qy_py[i], eval_loss_qy_py_std[i], eval_loss_qy_py_err[i], eval_loss_qy_py_err_std[i], \
                                    eval_loss_qz_pz[i], eval_loss_qz_pz_std[i], \
                                    eval_loss_qy_py_e[i], eval_loss_qy_py_e_std[i], eval_loss_qy_py_err_e[i], eval_loss_qy_py_err_e_std[i], \
                            ...
                            if args.cap_dim is not None:
                                ...
                                if pair_exist:
                                    text_log += "[%ld] %.6f (+- %.6f) ; %.6f (+- %.6f) %.6f (+- %.6f) %.6f (+- %.6f) %% %.6f (+- %.6f) , %.6f (+- %.6f) %.6f (+- %.6f) %% %.6f (+- %.6f) ; " \
                                        ...
                                        min_eval_loss_gv_src_src, min_eval_loss_gv_src_trg, min_eval_loss_mcdpow_src_trg, min_eval_loss_mcdpow_src_trg_std, \
                                        min_eval_loss_mcd_src_trg, min_eval_loss_mcd_src_trg_std, min_eval_loss_uv_src_trg, min_eval_loss_uv_src_trg_std, \
                                        min_eval_loss_f0_src_trg, min_eval_loss_f0_src_trg_std, min_eval_loss_lat_dist_rmse, min_eval_loss_lat_dist_rmse_std, \
                                        min_eval_loss_lat_dist_cossim, min_eval_loss_lat_dist_cossim_std)
                                else:
                                    text_log += "[%ld] %.6f (+- %.6f) ; %.6f (+- %.6f) %.6f (+- %.6f) %.6f (+- %.6f) %% %.6f (+- %.6f) , %.6f (+- %.6f) %.6f (+- %.6f) %% %.6f (+- %.6f) ; " \
                                        "%.6f (+- %.6f) %.6f (+- %.6f) ; %.6f (+- %.6f) dB %.6f (+- %.6f) dB , %.6f (+- %.6f) %% " \
            ...
            if True:
                ...
                save_checkpoint(args.expdir, model_encoder_mcep, model_decoder_mcep, model_encoder_excit, \
                    model_decoder_excit, optimizer, numpy_random_state, torch_random_state, epoch_idx + 1)
            total = 0
            iter_count = 0
            for i in range(args.n_half_cyc):
                loss_elbo[i] = []
        ...
        if epoch_idx < args.epoch_count:
            ...
            if f_ss > 0:
                ...
                for i in range(0,args.n_half_cyc,2):
                    ...
                    idx_in += 1
                    if args.ar_dec:
                        batch_mcep_rec[i], h_mcep[i], x_in[i] = model_decoder_mcep(batch_sc_in[idx_in], z[i], x_in[i], h=h_mcep[i], outpad_right=outpad_rights[idx_in], do=True)
                        batch_mcep_cv[i_cv], h_mcep_cv[i_cv], x_in_cv[i_cv] = model_decoder_mcep(batch_sc_cv_in[i_cv], z[i], x_in_cv[i_cv], h=h_mcep_cv[i_cv], outpad_right=outpad_rights[idx_in], do=True)
                    else:
                        batch_mcep_rec[i], h_mcep[i] = model_decoder_mcep(batch_sc_in[idx_in], z[i], h=h_mcep[i], outpad_right=outpad_rights[idx_in], do=True)
                        batch_mcep_cv[i_cv], h_mcep_cv[i_cv] = model_decoder_mcep(batch_sc_cv_in[i_cv], z[i], h=h_mcep_cv[i_cv], outpad_right=outpad_rights[idx_in], do=True)
                    ...
                    if args.ar_f0:
                        batch_lf0_rec[i], h_lf0[i], e_in[i] = model_decoder_excit(batch_sc_in[idx_in], z_e[i], e_in[i], h=h_lf0[i], outpad_right=outpad_rights[idx_in], do=True)
                        batch_lf0_cv[i_cv], h_lf0_cv[i_cv], e_in_cv[i_cv] = model_decoder_excit(batch_sc_cv_in[i_cv], z_e[i], e_in_cv[i_cv], h=h_lf0_cv[i_cv], outpad_right=outpad_rights[idx_in], do=True)
                    else:
                        batch_lf0_rec[i], h_lf0[i] = model_decoder_excit(batch_sc_in[idx_in], z_e[i], h=h_lf0[i], outpad_right=outpad_rights[idx_in], do=True)
                        batch_lf0_cv[i_cv], h_lf0_cv[i_cv] = model_decoder_excit(batch_sc_cv_in[i_cv], z_e[i], h=h_lf0_cv[i_cv], outpad_right=outpad_rights[idx_in], do=True)
                    feat_len = qy_logits[i].shape[1]
                    idx_in_1 = idx_in-1
                    ...
                    if args.detach:
                        ...
                        if args.ar_enc:
                                qy_logits[j], qz_alpha[j], h_z[j], yz_in[j] \
                                    = model_encoder_mcep(torch.cat((batch_feat_cv_in[i_cv], cv_mcep), 2), yz_in=yz_in[j], h=h_z[j], outpad_right=outpad_rights[idx_in], do=True)
                            ...
                            qy_logits[j], qz_alpha[j], h_z[j] \
                                = model_encoder_mcep(torch.cat((batch_feat_cv_in[i_cv], cv_mcep), 2), h=h_z[j], outpad_right=outpad_rights[idx_in], do=True)
                        ...
                        if args.ar_enc:
                                qy_logits[j], qz_alpha[j], h_z[j], yz_in[j] \
                                    = model_encoder_mcep(torch.cat((batch_feat_cv_in[i_cv], batch_mcep_cv[i_cv]), 2), yz_in=yz_in[j], h=h_z[j], outpad_right=outpad_rights[idx_in], do=True)
                                qy_logits_e[j], qz_alpha_e[j], h_z_e[j], yz_in_e[j] \
                            ...
                            qy_logits[j], qz_alpha[j], h_z[j] \
                                = model_encoder_mcep(torch.cat((batch_feat_cv_in[i_cv], batch_mcep_cv[i_cv]), 2), h=h_z[j], outpad_right=outpad_rights[idx_in], do=True)
                            qy_logits_e[j], qz_alpha_e[j], h_z_e[j] \
                    ...
                    if args.n_half_cyc > 1:
                        ...
                        idx_in += 1
                        if args.ar_dec:
                            batch_mcep_rec[j], h_mcep[j], x_in[j] = model_decoder_mcep(batch_sc_in[idx_in], z[j], x_in[j], h=h_mcep[j], outpad_right=outpad_rights[idx_in], do=True)
                        else:
                            batch_mcep_rec[j], h_mcep[j] = model_decoder_mcep(batch_sc_in[idx_in], z[j], h=h_mcep[j], outpad_right=outpad_rights[idx_in], do=True)
                        ...
                        if args.ar_f0:
                            batch_lf0_rec[j], h_lf0[j], e_in[j] = model_decoder_excit(batch_sc_in[idx_in], z_e[j], e_in[j], h=h_lf0[j], outpad_right=outpad_rights[idx_in], do=True)
                        else:
                            batch_lf0_rec[j], h_lf0[j] = model_decoder_excit(batch_sc_in[idx_in], z_e[j], h=h_lf0[j], outpad_right=outpad_rights[idx_in], do=True)
                        feat_len = qy_logits[j].shape[1]
                        idx_in_1 = idx_in-1
                ...
                for i in range(0,args.n_half_cyc,2):
                    ...
                    if i > 0:
                        ...
                        if args.detach:
                            if args.ar_enc:
                                ...
                                qy_logits[i], qz_alpha[i], h_z[i] \
                                    = model_encoder_mcep(torch.cat((batch_feat_in[idx_in][:,:,:args.excit_dim], batch_mcep_rec[i-1].detach()), 2), outpad_right=outpad_rights[idx_in], do=True)
                                qy_logits_e[i], qz_alpha_e[i], h_z_e[i] \
                                    = model_encoder_excit(torch.cat((batch_lf0_rec[i-1][:,:,:args.excit_dim].detach(), batch_feat_in[idx_in][:,:,args.excit_dim:]), 2), outpad_right=outpad_rights[idx_in], do=True)
                        else:
                            if args.ar_enc:
                                qy_logits[i], qz_alpha[i], h_z[i], yz_in[i] \
                                    = model_encoder_mcep(torch.cat((batch_feat_in[idx_in][:,:,:args.excit_dim], batch_mcep_rec[i-1]), 2), yz_in=yz_in_, outpad_right=outpad_rights[idx_in], do=True)
                                qy_logits_e[i], qz_alpha_e[i], h_z_e[i], yz_in_e[i] \
                                    = model_encoder_excit(torch.cat((batch_lf0_rec[i-1][:,:,:args.excit_dim], batch_feat_in[idx_in][:,:,args.excit_dim:]), 2), yz_in=yz_in_e_, outpad_right=outpad_rights[idx_in], do=True)
                            else:
                                qy_logits[i], qz_alpha[i], h_z[i] \
                                    = model_encoder_mcep(torch.cat((batch_feat_in[idx_in][:,:,:args.excit_dim], batch_mcep_rec[i-1]), 2), outpad_right=outpad_rights[idx_in], do=True)
                                qy_logits_e[i], qz_alpha_e[i], h_z_e[i] \
                                    = model_encoder_excit(torch.cat((batch_lf0_rec[i-1][:,:,:args.excit_dim], batch_feat_in[idx_in][:,:,args.excit_dim:]), 2), outpad_right=outpad_rights[idx_in], do=True)
                        i_1 = i-1
                        idx_in_1 = idx_in-1
                        feat_len = batch_mcep_rec[i_1].shape[1]
                        batch_mcep_rec[i_1] = batch_mcep_rec[i_1][:,outpad_lefts[idx_in_1]:feat_len-outpad_rights[idx_in_1]]
                        ...
                        if args.ar_enc:
                            qy_logits[i], qz_alpha[i], h_z[i], yz_in[i] = model_encoder_mcep(batch_feat_in[idx_in], yz_in_, outpad_right=outpad_rights[idx_in], do=True)
                            qy_logits_e[i], qz_alpha_e[i], h_z_e[i], yz_in_e[i] = model_encoder_excit(batch_feat_in[idx_in], yz_in_e_, outpad_right=outpad_rights[idx_in], do=True)
                        else:
                    ...
                    idx_in += 1
                    if args.ar_dec:
                        batch_mcep_rec[i], h_mcep[i], x_in[i] = model_decoder_mcep(batch_sc_in[idx_in], z[i], x_in_, outpad_right=outpad_rights[idx_in], do=True)
                        batch_mcep_cv[i_cv], h_mcep_cv[i_cv], x_in_cv[i_cv] = model_decoder_mcep(batch_sc_cv_in[i_cv], z[i], x_in_, outpad_right=outpad_rights[idx_in], do=True)
                    else:
                        batch_mcep_rec[i], h_mcep[i] = model_decoder_mcep(batch_sc_in[idx_in], z[i], outpad_right=outpad_rights[idx_in], do=True)
                        batch_mcep_cv[i_cv], h_mcep_cv[i_cv] = model_decoder_mcep(batch_sc_cv_in[i_cv], z[i], outpad_right=outpad_rights[idx_in], do=True)
                    eps_e = torch.empty_like(qz_alpha_e[i][:,:,:args.lat_dim_e])
                    eps_e.uniform_(eps_1,1)
                    z_e[i] = qz_alpha_e[i][:,:,:args.lat_dim_e] - torch.exp(qz_alpha_e[i][:,:,args.lat_dim_e:]) * eps_e.sign() * torch.log1p(-eps_e.abs()) # sampling laplace
                    if args.ar_f0:
                        batch_lf0_rec[i], h_lf0[i], e_in[i] = model_decoder_excit(batch_sc_in[idx_in], z_e[i], e_in_, outpad_right=outpad_rights[idx_in], do=True)
                        batch_lf0_cv[i_cv], h_lf0_cv[i_cv], e_in_cv[i_cv] = model_decoder_excit(batch_sc_cv_in[i_cv], z_e[i], e_in_, outpad_right=outpad_rights[idx_in], do=True)
                    else:
                        batch_lf0_rec[i], h_lf0[i] = model_decoder_excit(batch_sc_in[idx_in], z_e[i], outpad_right=outpad_rights[idx_in], do=True)
                        batch_lf0_cv[i_cv], h_lf0_cv[i_cv] = model_decoder_excit(batch_sc_cv_in[i_cv], z_e[i], outpad_right=outpad_rights[idx_in], do=True)
                    feat_len = qy_logits[i].shape[1]
                    idx_in_1 = idx_in-1
                    qy_logits[i] = qy_logits[i][:,outpad_lefts[idx_in_1]:feat_len-outpad_rights[idx_in_1]]
                    qz_alpha[i] = qz_alpha[i][:,outpad_lefts[idx_in_1]:feat_len-outpad_rights[idx_in_1]]
                    ...
                    if args.detach:
                        ...
                        if args.ar_enc:
                            qy_logits[j], qz_alpha[j], h_z[j], yz_in[j] \
                                = model_encoder_mcep(torch.cat((batch_feat_cv_in[i_cv], cv_mcep), 2), yz_in=yz_in_, outpad_right=outpad_rights[idx_in], do=True)
                            qy_logits_e[j], qz_alpha_e[j], h_z_e[j], yz_in_e[j] \
                            ...
                            qy_logits[j], qz_alpha[j], h_z[j] \
                                = model_encoder_mcep(torch.cat((batch_feat_cv_in[i_cv], cv_mcep), 2), outpad_right=outpad_rights[idx_in], do=True)
                            qy_logits_e[j], qz_alpha_e[j], h_z_e[j] \
                        ...
                        if args.ar_enc:
                            qy_logits[j], qz_alpha[j], h_z[j], yz_in[j] \
                                = model_encoder_mcep(torch.cat((batch_feat_cv_in[i_cv], batch_mcep_cv[i_cv]), 2), yz_in=yz_in_, outpad_right=outpad_rights[idx_in], do=True)
                            qy_logits_e[j], qz_alpha_e[j], h_z_e[j], yz_in_e[j] \
                            ...
                            qy_logits[j], qz_alpha[j], h_z[j] \
                                = model_encoder_mcep(torch.cat((batch_feat_cv_in[i_cv], batch_mcep_cv[i_cv]), 2), outpad_right=outpad_rights[idx_in], do=True)
                            qy_logits_e[j], qz_alpha_e[j], h_z_e[j] \
                    ...
                    if args.n_half_cyc > 1:
                        ...
                        idx_in += 1
                        if args.ar_dec:
                            batch_mcep_rec[j], h_mcep[j], x_in[j] = model_decoder_mcep(batch_sc_in[idx_in], z[j], x_in_, outpad_right=outpad_rights[idx_in], do=True)
                        else:
                            batch_mcep_rec[j], h_mcep[j] = model_decoder_mcep(batch_sc_in[idx_in], z[j], outpad_right=outpad_rights[idx_in], do=True)
                        eps_e = torch.empty_like(qz_alpha_e[j][:,:,:args.lat_dim_e])
                        eps_e.uniform_(eps_1,1)
                        z_e[j] = qz_alpha_e[j][:,:,:args.lat_dim_e] - torch.exp(qz_alpha_e[j][:,:,args.lat_dim_e:]) * eps_e.sign() * torch.log1p(-eps_e.abs()) # sampling laplace
                        if args.ar_f0:
                            batch_lf0_rec[j], h_lf0[j], e_in[j] = model_decoder_excit(batch_sc_in[idx_in], z_e[j], e_in_, outpad_right=outpad_rights[idx_in], do=True)
                        else:
            ...
            if len(idx_select) > 0:
                ...
                for j in range(len(idx_select)):
                    ...
                    for i in range(args.n_half_cyc):
                        ...
                        if args.cap_dim is not None:
                            batch_loss_px_select += torch.mean(100*criterion_l1(uvcap_est, uvcap_select)) \
                        ...
                        if i % 2 == 0:
                            ...
                            if flens_utt > 1:
                                batch_loss_px_select += torch.sqrt(torch.mean(criterion_l2(torch.exp(batch_lf0_cv[i//2][k,:flens_utt,1]), \
                                                                torch.exp(batch_feat_cv[i//2][k,:flens_utt,1]))))
                            else:
                            ...
                            batch_loss_qy_py_err_select += torch.mean(100*torch.sum(criterion_l1(F.softmax(qy_logits_select_, dim=-1), batch_sc_onehot_), -1)) \
                            ...
                            batch_loss_qy_py_ce_select += torch.mean(criterion_ce(qy_logits_select_, sc_cv_select_)) \
                                                                + torch.mean(criterion_ce(qy_logits_e_select_, sc_cv_select_))
                            batch_loss_qy_py_err_select += torch.mean(100*torch.sum(criterion_l1(F.softmax(qy_logits_select_, dim=-1), sc_cv_select_onehot_), -1)) \
                ...
                batch_loss += batch_loss_px_select + batch_loss_qz_pz_kl_select + batch_loss_qy_py_ce_select + batch_loss_qy_py_err_select
    ...
    torch.save({"model_encoder_mcep": model_encoder_mcep.state_dict(),
                "model_decoder_mcep": model_decoder_mcep.state_dict(),
                "model_encoder_excit": model_encoder_excit.state_dict(),
                "model_decoder_excit": model_decoder_excit.state_dict()}, args.expdir + "/checkpoint-final.pkl")
