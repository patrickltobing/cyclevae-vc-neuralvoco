...
def main():
    ...
    args.cap_dim = read_hdf5(args.stats, "/scale_feat_mceplf0cap")[3:args.full_excit_dim].shape[0]
    ...
    model_waveform = GRU_WAVE_DECODER_DUALGRU_COMPACT_MBAND_CF(
        feat_dim=n_spk*2+args.lat_dim+args.lat_dim_e+args.mel_dim,
                                ...
                                if args.spkidtr_dim > 0:
                                    batch_x_c_output[i], batch_x_f_output[i], h_x[i], h_x_2[i], h_f[i] \
                                        = model_waveform(z_cat, batch_x_c_prev, batch_x_f_prev, batch_x_c,
                                            spk_code=spk_code_in, spk_aux=batch_spk, aux=batch_melsp_rec[i], x_c_lpc=batch_x_c_lpc, x_f_lpc=batch_x_f_lpc,
                                                h=h_x[i], h_2=h_x_2[i], h_f=h_f[i], outpad_left=outpad_lefts[idx_in], outpad_right=outpad_rights[idx_in])
                                else:
                                    batch_x_c_output[i], batch_x_f_output[i], h_x[i], h_x_2[i], h_f[i] \
                                        = model_waveform(z_cat, batch_x_c_prev, batch_x_f_prev, batch_x_c,
                                            spk_code=batch_sc_in[idx_in], spk_aux=batch_spk, aux=batch_melsp_rec[i], x_c_lpc=batch_x_c_lpc, x_f_lpc=batch_x_f_lpc,
                                                h=h_x[i], h_2=h_x_2[i], h_f=h_f[i], outpad_left=outpad_lefts[idx_in], outpad_right=outpad_rights[idx_in])
                            else:
                                if args.spkidtr_dim > 0:
                                    batch_x_c_output[i], batch_x_f_output[i], h_x[i], h_x_2[i], h_f[i] \
                                        = model_waveform(z_cat, batch_x_c_prev, batch_x_f_prev, batch_x_c,
                                            spk_code=spk_code_in, spk_aux=batch_spk, aux=batch_melsp_rec[i],
                                                h=h_x[i], h_2=h_x_2[i], h_f=h_f[i], outpad_left=outpad_lefts[idx_in], outpad_right=outpad_rights[idx_in])
                                else:
                                    batch_x_c_output[i], batch_x_f_output[i], h_x[i], h_x_2[i], h_f[i] \
                                        = model_waveform(z_cat, batch_x_c_prev, batch_x_f_prev, batch_x_c,
                                            spk_code=batch_sc_in[idx_in], spk_aux=batch_spk, aux=batch_melsp_rec[i],
                                                h=h_x[i], h_2=h_x_2[i], h_f=h_f[i], outpad_left=outpad_lefts[idx_in], outpad_right=outpad_rights[idx_in])
                                ...
                                if args.lpc > 0:
                                    if args.spkidtr_dim > 0:
                                        batch_x_c_output[j], batch_x_f_output[j], h_x[j], h_x_2[j], h_f[j] \
                                            = model_waveform(z_cat, batch_x_c_prev, batch_x_f_prev, batch_x_c,
                                                 spk_code=spk_code_in, spk_aux=batch_spk, aux=batch_melsp_rec[j], x_c_lpc=batch_x_c_lpc, x_f_lpc=batch_x_f_lpc,
                                                    h=h_x[j], h_2=h_x_2[j], h_f=h_f[j], outpad_left=outpad_lefts[idx_in], outpad_right=outpad_rights[idx_in])
                                    else:
                                        batch_x_c_output[j], batch_x_f_output[j], h_x[j], h_x_2[j], h_f[j] \
                                            = model_waveform(z_cat, batch_x_c_prev, batch_x_f_prev, batch_x_c,
                                                 spk_code=batch_sc_in[idx_in], spk_aux=batch_spk, aux=batch_melsp_rec[j], x_c_lpc=batch_x_c_lpc, x_f_lpc=batch_x_f_lpc,
                                                    h=h_x[j], h_2=h_x_2[j], h_f=h_f[j], outpad_left=outpad_lefts[idx_in], outpad_right=outpad_rights[idx_in])
                                else:
                                    if args.spkidtr_dim > 0:
                                        batch_x_c_output[j], batch_x_f_output[j], h_x[j], h_x_2[j], h_f[j] \
                                            = model_waveform(z_cat, batch_x_c_prev, batch_x_f_prev, batch_x_c,
                                                 spk_code=spk_code_in, spk_aux=batch_spk, aux=batch_melsp_rec[j],
                                                    h=h_x[j], h_2=h_x_2[j], h_f=h_f[j], outpad_left=outpad_lefts[idx_in], outpad_right=outpad_rights[idx_in])
                                    else:
                                        batch_x_c_output[j], batch_x_f_output[j], h_x[j], h_x_2[j], h_f[j] \
                                            = model_waveform(z_cat, batch_x_c_prev, batch_x_f_prev, batch_x_c,
                                                 spk_code=batch_sc_in[idx_in], spk_aux=batch_spk, aux=batch_melsp_rec[j],
                                                    h=h_x[j], h_2=h_x_2[j], h_f=h_f[j], outpad_left=outpad_lefts[idx_in], outpad_right=outpad_rights[idx_in])
                    else:
                        pair_flag = False
