    ...
    while True:
        ...
        for idx, batch in enumerate(dataloader):
            ...
            if spcidx:
                ...
                feat_cv_full = batch['cv_src_full'][:,:max_flen_full].to(device)
                ...
                if spcidx:
                    yield feat, feat_trg, sc, sc_cv, feat_cv, c_idx, idx, featfiles, f_bs, f_ss, flens, \
                        n_batch_utt, del_index_utt, max_flen, spk_cv, file_src_trg_flag, spcidx_src, \
                            spcidx_src_trg, flens_spc_src, flens_spc_src_trg, feat_full, feat_cv_full, sc_full, sc_cv_full
                else:
                    yield feat, feat_trg, sc, sc_cv, feat_cv, c_idx, idx, featfiles, f_bs, f_ss, flens, \
                        n_batch_utt, del_index_utt, max_flen, spk_cv, file_src_trg_flag, spcidx_src, \
...
def save_checkpoint(checkpoint_dir, model_encoder, model_decoder, model_classifier, model_post,
...
def main():
    ...
    parser.add_argument("--causal_conv_post", default=False,
                        type=strtobool, help="causal input conv. gru decoder")
    # other setting
    ...
    mean_stats_post = torch.cat((mean_stats[:2], mean_stats[args.excit_dim:]))
    scale_stats_post = torch.cat((scale_stats[:2], scale_stats[args.excit_dim:]))
    ...
    if torch.cuda.is_available():
        ...
        mean_stats_post = mean_stats_post.cuda()
        scale_stats_post = scale_stats_post.cuda()
    else:
    ...
    model_post.scale_in.weight = torch.nn.Parameter(torch.unsqueeze(torch.diag(1.0/scale_stats_post.data),2))
    model_post.scale_in.bias = torch.nn.Parameter(-(mean_stats_post.data/scale_stats_post.data))
    ...
    model_encoder.load_state_dict(gen_model["model_encoder"])
    model_decoder.load_state_dict(gen_model["model_decoder"])
    ...
    first_pad_left = (enc_pad_left + dec_pad_left + post_pad_left)*args.n_half_cyc
    first_pad_right = (enc_pad_right + dec_pad_right + post_pad_right)*args.n_half_cyc
    ...
    batch_mcep_rec_post = [None]*n_rec
    batch_mcep_rec_sc = [None]*n_rec
    batch_mcep_cv = [None]*n_cv
    batch_mcep_cv_post = [None]*n_cv
    batch_mcep_cv_sc = [None]*n_cv
    h_z = [None]*n_rec
    h_mcep = [None]*n_rec
    h_mcep_post = [None]*n_rec
    h_mcep_sc = [None]*n_rec
    h_mcep_cv = [None]*n_cv
    h_mcep_cv_post = [None]*n_cv
    ...
    first_pad_left_eval = (enc_pad_left + dec_pad_left + post_pad_left)*n_half_cyc_eval
    first_pad_right_eval = (enc_pad_right + dec_pad_right + post_pad_right)*n_half_cyc_eval
    ...
    first_pad_left_eval_utt = enc_pad_left + dec_pad_left + post_pad_left
    first_pad_right_eval_utt = enc_pad_right + dec_pad_right + post_pad_right
    ...
    while epoch_idx < args.epoch_count:
        ...
        if c_idx < 0: # summarize epoch
            ...
            for i in range(args.n_half_cyc):
                if i % 2 == 0:
                    ...
                    text_log += "[%ld] %.6f (+- %.6f) ; %.6f (+- %.6f) ; "\
                        ...
                        np.mean(loss_px[i]), np.std(loss_px[i]), np.mean(loss_mcep_rec[i]), np.std(loss_mcep_rec[i]), \
                        np.mean(loss_powmcep[i]), np.std(loss_powmcep[i]), np.mean(loss_mcep[i]), np.std(loss_mcep[i]))
            ...
            while True:
                with torch.no_grad():
                    ...
                    batch_feat_data, batch_feat_trg_data, batch_sc_data, batch_sc_cv_data, batch_feat_cv_data, c_idx, utt_idx, featfile, \
                        f_bs, f_ss, flens, n_batch_utt, del_index_utt, max_flen, spk_cv, src_trg_flag, \
                            spcidx_src, spcidx_src_trg, flens_spc_src, flens_spc_src_trg, \
                                batch_feat_data_full, batch_feat_cv_data_full, batch_sc_data_full, batch_sc_cv_data_full = next(generator_eval)
                    if c_idx < 0:
                        break
                    ...
                    for i in range(i_end):
                        i_cv = i//(n_half_cyc_eval*3)
                        if i % 3 == 0: #enc
                            if not flag_cv_enc:
                                if f_ss_first_pad_left >= 0 and f_es_first_pad_right <= max_flen: # pad left and right available
                                    batch_feat_in[i_in] = batch_feat_data[:,f_ss_first_pad_left:f_es_first_pad_right]
                                elif f_es_first_pad_right <= max_flen: # pad right available, left need additional replicate
                                    batch_feat_in[i_in] = F.pad(batch_feat_data[:,:f_es_first_pad_right].transpose(1,2), (-f_ss_first_pad_left,0), "replicate").transpose(1,2)
                                elif f_ss_first_pad_left >= 0: # pad left available, right need additional replicate
                                    batch_feat_in[i_in] = F.pad(batch_feat_data[:,f_ss_first_pad_left:max_flen].transpose(1,2), (0,f_es_first_pad_right-max_flen), "replicate").transpose(1,2)
                                else: # pad left and right need additional replicate
                                    batch_feat_in[i_in] = F.pad(batch_feat_data[:,:max_flen].transpose(1,2), (-f_ss_first_pad_left,f_es_first_pad_right-max_flen), "replicate").transpose(1,2)
                                i_in += 1
                                flag_cv_enc = True
                            else:
                                if f_ss_first_pad_left >= 0 and f_es_first_pad_right <= max_flen: # pad left and right available
                            ...
                            if f_ss_first_pad_left >= 0 and f_es_first_pad_right <= max_flen: # pad left and right available
                                ...
                                if flag_cv:
                                    ...
                                    if i % 3 == 2:
                                        batch_feat_in[i_in] = F.pad(batch_feat_data[:,f_ss_first_pad_left:max_flen].transpose(1,2), (0,f_es_first_pad_right-max_flen), "replicate").transpose(1,2)
                                        i_in += 1
                                        batch_feat_cv_in[i_cv_enc] = F.pad(batch_feat_cv_data[:,f_ss_first_pad_left:max_flen].transpose(1,2), (0,diff_pad), "replicate").transpose(1,2)
                                        i_cv_enc += 1
                                        flag_cv = False
                                else:
                                    if (i + 1) % 6 == 0:
                                ...
                                if flag_cv:
                                    ...
                                    if i % 3 == 2:
                                        batch_feat_in[i_in] = F.pad(batch_feat_data[:,:max_flen].transpose(1,2), (-f_ss_first_pad_left,f_es_first_pad_right-max_flen), "replicate").transpose(1,2)
                                        i_in += 1
                                        batch_feat_cv_in[i_cv_enc] = F.pad(batch_feat_cv_data[:,:max_flen].transpose(1,2), (-f_ss_first_pad_left,diff_pad), "replicate").transpose(1,2)
                                        i_cv_enc += 1
                                        flag_cv = False
                                else:
                                    if (i + 1) % 6 == 0:
                    ...
                    if f_ss > 0:
                        ...
                        for i in range(0,n_half_cyc_eval,2):
                            ...
                            if len(del_index_utt) > 0:
                                ...
                                h_mcep[i] = torch.FloatTensor(np.delete(h_mcep[i].cpu().data.numpy(), \
                                                                del_index_utt, axis=1)).to(device)
                                h_mcep_post[i] = torch.FloatTensor(np.delete(h_mcep_post[i].cpu().data.numpy(), \
                                                                del_index_utt, axis=1)).to(device)
                                ...
                                h_mcep_cv[i_cv] = torch.FloatTensor(np.delete(h_mcep_cv[i_cv].cpu().data.numpy(), \
                                                                del_index_utt, axis=1)).to(device)
                                h_mcep_cv_post[i_cv] = torch.FloatTensor(np.delete(h_mcep_cv_post[i_cv].cpu().data.numpy(), \
                                                                del_index_utt, axis=1)).to(device)
                                ...
                                if n_half_cyc_eval > 1:
                                    h_mcep[j] = torch.FloatTensor(np.delete(h_mcep[j].cpu().data.numpy(), \
                                                                    del_index_utt, axis=1)).to(device)
                                    h_mcep_post[j] = torch.FloatTensor(np.delete(h_mcep_post[j].cpu().data.numpy(), \
                                                                    del_index_utt, axis=1)).to(device)
                            ...
                            if i > 0:
                                ...
                                i_1 = i-1
                                if post_pad_right > 0:
                                    cyc_rec_mcep = batch_mcep_rec[i_1][:,post_pad_left:-post_pad_right]
                                else:
                                    cyc_rec_mcep = batch_mcep_rec[i_1][:,post_pad_left:]
                                _, _, z[i], h_z[i] \
                                    = model_encoder(torch.cat((batch_feat_in[i_in][:,:,:args.excit_dim], cyc_rec_mcep), 2), h=h_z[i], outpad_right=outpad_rights_eval[idx_in], sampling=False)
                            else:
                                _, _, z[i], h_z[i] = model_encoder(batch_feat_in[i_in], h=h_z[i], outpad_right=outpad_rights_eval[idx_in], sampling=False)
                            ...
                            batch_mcep_cv[i_cv], h_mcep_cv[i_cv] = model_decoder(batch_sc_cv_in[i_cv_in], z[i], h=h_mcep_cv[i_cv], outpad_right=outpad_rights_eval[idx_in])
                            ## post
                            idx_in += 1
                            i_in += 1
                            i_cv_in += 1
                            batch_mcep_rec_post[i], h_mcep_post[i] = model_post(batch_sc_in[idx_in], batch_mcep_rec[i],
                                                e=batch_feat_in[i_in][:,:,:2], outpad_right=outpad_rights_eval[idx_in], h=h_mcep_post[i])
                            batch_mcep_cv_post[i_cv], h_mcep_cv_post[i_cv] = model_post(batch_sc_cv_in[i_cv_in], batch_mcep_cv[i_cv],
                                                e=batch_feat_cv_in[i_cv_enc][:,:,:2], outpad_right=outpad_rights_eval[idx_in], h=h_mcep_cv_post[i_cv])
                            feat_len = batch_mcep_rec_post[i].shape[1]
                            batch_mcep_rec_post[i] = batch_mcep_rec_post[i][:,outpad_lefts_eval[idx_in]:feat_len-outpad_rights_eval[idx_in]]
                            batch_mcep_cv_post[i_cv] = batch_mcep_cv_post[i_cv][:,outpad_lefts_eval[idx_in]:feat_len-outpad_rights_eval[idx_in]]
                            batch_mcep_rec_sc[i], h_mcep_sc[i] = model_classifier(feat=batch_mcep_rec_post[i], h=h_mcep_sc[i])
                            batch_mcep_cv_sc[i_cv], h_mcep_cv_sc[i_cv] = model_classifier(feat=batch_mcep_cv_post[i_cv], h=h_mcep_cv_sc[i_cv])
                            if n_half_cyc_eval > 1:
                                ## cyclic reconstruction
                                idx_in += 1
                                i_cv_enc += 1
                                if post_pad_right > 0:
                                    cv_mcep = batch_mcep_cv[i_cv][:,post_pad_left:-post_pad_right]
                                else:
                                    cv_mcep = batch_mcep_cv[i_cv][:,post_pad_left:]
                                _, _, z[j], h_z[j] \
                                    = model_encoder(torch.cat((batch_feat_cv_in[i_cv_enc], cv_mcep), 2), h=h_z[j], outpad_right=outpad_rights_eval[idx_in], sampling=False)
                                idx_in += 1
                                batch_mcep_rec[j], h_mcep[j] = model_decoder(batch_sc_in[idx_in], z[j], h=h_mcep[j], outpad_right=outpad_rights_eval[idx_in])
                                ## post
                                idx_in += 1
                                i_in += 1
                                batch_mcep_rec_post[j], h_mcep_post[j] = model_post(batch_sc_in[idx_in], batch_mcep_rec[j],
                                                    e=batch_feat_in[i_in][:,:,:2], outpad_right=outpad_rights_eval[idx_in], h=h_mcep_post[j])
                                batch_mcep_rec_post[j] = batch_mcep_rec_post[j][:,outpad_lefts_eval[idx_in]:batch_mcep_rec_post[j].shape[1]-outpad_rights_eval[idx_in]]
                                batch_mcep_rec_sc[j], h_mcep_sc[j] = model_classifier(feat=batch_mcep_rec_post[j], h=h_mcep_sc[j])
                    else:
                        pair_flag = False
                        ...
                        batch_sc_data_full = F.pad(batch_sc_data_full.unsqueeze(1).float(), (dec_pad_left+post_pad_left,dec_pad_right+post_pad_right), "replicate").squeeze(1).long()
                        batch_sc_cv_data_full = F.pad(batch_sc_cv_data_full.unsqueeze(1).float(), (dec_pad_left+post_pad_left,dec_pad_right+post_pad_right), "replicate").squeeze(1).long()
                        trj_src_src, _ = model_decoder(batch_sc_data_full, trj_lat_src)
                        trj_src_trg, _ = model_decoder(batch_sc_cv_data_full, trj_lat_src)
                        if dec_pad_right > 0:
                            batch_sc_data_full = batch_sc_data_full[:,dec_pad_left:-dec_pad_right]
                            batch_sc_cv_data_full = batch_sc_cv_data_full[:,dec_pad_left:-dec_pad_right]
                            batch_feat_data_full = batch_feat_data_full[:,enc_pad_left+dec_pad_left:-(enc_pad_right+dec_pad_right)]
                        else:
                            batch_sc_data_full = batch_sc_data_full[:,dec_pad_left:]
                            batch_sc_cv_data_full = batch_sc_cv_data_full[:,dec_pad_left:]
                            if enc_pad_right > 0:
                                batch_feat_data_full = batch_feat_data_full[:,enc_pad_left+dec_pad_left:-enc_pad_right]
                            else:
                                batch_feat_data_full = batch_feat_data_full[:,enc_pad_left+dec_pad_left:]
                        batch_feat_cv_data_full = F.pad(batch_feat_cv_data_full.transpose(1,2), (post_pad_left,post_pad_right), "replicate").transpose(1,2)
                        trj_src_src, _ = model_post(batch_sc_data_full, trj_src_src, e=batch_feat_data_full[:,:,:2])
                        trj_src_trg, _ = model_post(batch_sc_cv_data_full, trj_src_trg, e=batch_feat_cv_data_full[:,:,:2])
                        for k in range(n_batch_utt):
                        ...
                        if pair_flag:
                            for k in range(n_batch_utt):
                                if src_trg_flag[k]:
                                    ...
                                    logging.info('acc cv %s %s %.3f dB %.3f dB' % (featfile[k], \
                        ...
                        for i in range(0,n_half_cyc_eval,2):
                            ...
                            if i > 0:
                                ...
                                if post_pad_right > 0:
                                    ...
                                    cyc_rec_mcep = batch_mcep_rec[i_1][:,post_pad_left:]
                                _, _, z[i], h_z[i] \
                                    = model_encoder(torch.cat((batch_feat_in[i_in][:,:,:args.excit_dim], cyc_rec_mcep), 2), outpad_right=outpad_rights_eval[idx_in], sampling=False)
                            else:
                                _, _, z[i], h_z[i] = model_encoder(batch_feat_in[i_in], outpad_right=outpad_rights_eval[idx_in], sampling=False)
                            ...
                            batch_mcep_cv[i_cv], h_mcep_cv[i_cv] = model_decoder(batch_sc_cv_in[i_cv_in], z[i], outpad_right=outpad_rights_eval[idx_in])
                            ## post
                            idx_in += 1
                            i_in += 1
                            i_cv_in += 1
                            batch_mcep_rec_post[i], h_mcep_post[i] = model_post(batch_sc_in[idx_in], batch_mcep_rec[i],
                                                e=batch_feat_in[i_in][:,:,:2], outpad_right=outpad_rights_eval[idx_in])
                            batch_mcep_cv_post[i_cv], h_mcep_cv_post[i_cv] = model_post(batch_sc_cv_in[i_cv_in], batch_mcep_cv[i_cv],
                                                e=batch_feat_cv_in[i_cv_enc][:,:,:2], outpad_right=outpad_rights_eval[idx_in])
                            feat_len = batch_mcep_rec_post[i].shape[1]
                            batch_mcep_rec_post[i] = batch_mcep_rec_post[i][:,outpad_lefts_eval[idx_in]:feat_len-outpad_rights_eval[idx_in]]
                            batch_mcep_cv_post[i_cv] = batch_mcep_cv_post[i_cv][:,outpad_lefts_eval[idx_in]:feat_len-outpad_rights_eval[idx_in]]
                            batch_mcep_rec_sc[i], h_mcep_sc[i] = model_classifier(feat=batch_mcep_rec_post[i])
                            batch_mcep_cv_sc[i_cv], h_mcep_cv_sc[i_cv] = model_classifier(feat=batch_mcep_cv_post[i_cv])
                            if n_half_cyc_eval > 1:
                                ## cyclic reconstruction
                                idx_in += 1
                                i_cv_enc += 1
                                if post_pad_right > 0:
                                    ...
                                    cv_mcep = batch_mcep_cv[i_cv][:,post_pad_left:]
                                _, _, z[j], h_z[j] \
                                    = model_encoder(torch.cat((batch_feat_cv_in[i_cv_enc], cv_mcep), 2), outpad_right=outpad_rights_eval[idx_in], sampling=False)
                                idx_in += 1
                                ...
                                batch_mcep_rec_post[j], h_mcep_post[j] = model_post(batch_sc_in[idx_in], batch_mcep_rec[j],
                                                    e=batch_feat_in[i_in][:,:,:2], outpad_right=outpad_rights_eval[idx_in])
                                batch_mcep_rec_post[j] = batch_mcep_rec_post[j][:,outpad_lefts_eval[idx_in]:batch_mcep_rec_post[j].shape[1]-outpad_rights_eval[idx_in]]
                                batch_mcep_rec_sc[j], h_mcep_sc[j] = model_classifier(feat=batch_mcep_rec_post[j])
                    ...
                    logging.info(batch_mcep_rec_post[0][i,:2,:4])
                    if n_half_cyc_eval > 1:
                        logging.info(batch_mcep_rec_post[1][i,:2,:4])
                    logging.info(batch_feat[i,:2,args.excit_dim:args.excit_dim+4])
                    logging.info(batch_mcep_cv_post[0][i,:2,:4])
                    ...
                    for i in range(n_half_cyc_eval):
                        mcep_est = batch_mcep_rec_post[i]
                        if i % 2 == 0:
                            mcep_cv = batch_mcep_cv_post[i//2]
                        ...
                        batch_loss_px_mcep_ = mcd_constant*(torch.mean(torch.sum(criterion_l1(mcep_est, powmcep), -1), -1) \
                                                + torch.mean(torch.mean(criterion_l1(mcep_est, powmcep), -1), -1) \
                                            + torch.sqrt(torch.mean(torch.sum(criterion_l2(mcep_est, powmcep), -1), -1)) \
                                                + torch.sqrt(torch.mean(torch.mean(criterion_l2(mcep_est, powmcep), -1), -1)))
                        ...
                        # elbo
                        batch_loss_elbo[i] = batch_loss_px_sum + batch_loss_sc_mcep_kl
                ...
                if i % 2 == 0:
                    if pair_exist:
                        text_log += "[%ld] %.6f (+- %.6f) ; %.6f (+- %.6f) ; "\
                            ...
                            eval_loss_mcd_src_trg, eval_loss_mcd_src_trg_std)
                    else:
                        text_log += "[%ld] %.6f (+- %.6f) ; %.6f (+- %.6f) %.6f (+- %.6f) ; "\
                            "%.6f (+- %.6f) %.6f (+- %.6f) ; %.6f (+- %.6f) dB %.6f (+- %.6f) dB ; " \
                            "%.6f %.6f n/a (+- n/a) dB n/a (+- n/a) dB ;; " % (i+1,
                            eval_loss_elbo[i], eval_loss_elbo_std[i], eval_loss_px[i], eval_loss_px_std[i],
                            eval_loss_mcep_rec[i], eval_loss_mcep_rec_std[i], eval_loss_mcep_cv[i//2], eval_loss_mcep_cv_std[i//2],
                            eval_loss_powmcep[i], eval_loss_powmcep_std[i], eval_loss_mcep[i], eval_loss_mcep_std[i],
                    ...
                    if i % 2 == 0:
                        if pair_exist:
                            text_log += "[%ld] %.6f (+- %.6f) ; %.6f (+- %.6f) ; "\
                                ...
                                min_eval_loss_mcd_src_trg, min_eval_loss_mcd_src_trg_std)
                        else:
                            text_log += "[%ld] %.6f (+- %.6f) ; %.6f (+- %.6f) ; "\
                                "%.6f (+- %.6f) %.6f (+- %.6f) ; %.6f (+- %.6f) dB %.6f (+- %.6f) dB ; " \
                                "%.6f %.6f n/a (+- n/a) dB n/a (+- n/a) dB ;; " % (i+1, \
            ...
            if True:
                ...
                save_checkpoint(args.expdir, model_encoder, model_decoder, model_classifier, model_post,
        ...
        if epoch_idx < args.epoch_count:
            ...
            for i in range(i_end):
                i_cv = i//(args.n_half_cyc*3)
                if i % 3 == 0: #enc
                    if not flag_cv_enc:
                        if f_ss_first_pad_left >= 0 and f_es_first_pad_right <= max_flen: # pad left and right available
                            batch_feat_in[i_in] = batch_feat[:,f_ss_first_pad_left:f_es_first_pad_right]
                        elif f_es_first_pad_right <= max_flen: # pad right available, left need additional replicate
                            batch_feat_in[i_in] = F.pad(batch_feat[:,:f_es_first_pad_right].transpose(1,2), (-f_ss_first_pad_left,0), "replicate").transpose(1,2)
                        elif f_ss_first_pad_left >= 0: # pad left available, right need additional replicate
                            batch_feat_in[i_in] = F.pad(batch_feat[:,f_ss_first_pad_left:max_flen].transpose(1,2), (0,f_es_first_pad_right-max_flen), "replicate").transpose(1,2)
                        else: # pad left and right need additional replicate
                            batch_feat_in[i_in] = F.pad(batch_feat[:,:max_flen].transpose(1,2), (-f_ss_first_pad_left,f_es_first_pad_right-max_flen), "replicate").transpose(1,2)
                        i_in += 1
                        flag_cv_enc = True
                    else:
                        if f_ss_first_pad_left >= 0 and f_es_first_pad_right <= max_flen: # pad left and right available
                            batch_feat_cv_in[i_cv_enc] = batch_feat_cv_data[i_cv][:,f_ss_first_pad_left:f_es_first_pad_right]
                        elif f_es_first_pad_right <= max_flen: # pad right available, left need additional replicate
                            batch_feat_cv_in[i_cv_enc] = F.pad(batch_feat_cv_data[i_cv][:,:f_es_first_pad_right].transpose(1,2), (-f_ss_first_pad_left,0), "replicate").transpose(1,2)
                        elif f_ss_first_pad_left >= 0: # pad left available, right need additional replicate
                            batch_feat_cv_in[i_cv_enc] = F.pad(batch_feat_cv_data[i_cv][:,f_ss_first_pad_left:max_flen].transpose(1,2), (0,f_es_first_pad_right-max_flen), "replicate").transpose(1,2)
                        else: # pad left and right need additional replicate
                            batch_feat_cv_in[i_cv_enc] = F.pad(batch_feat_cv_data[i_cv][:,:max_flen].transpose(1,2), (-f_ss_first_pad_left,f_es_first_pad_right-max_flen), "replicate").transpose(1,2)
                        i_cv_enc += 1
                        flag_cv_enc = False
                    f_ss_first_pad_left += enc_pad_left
                    f_es_first_pad_right -= enc_pad_right
                    ...
                    if f_ss_first_pad_left >= 0 and f_es_first_pad_right <= max_flen: # pad left and right available
                        ...
                        if flag_cv:
                            ...
                            if i % 3 == 2:
                                batch_feat_in[i_in] = F.pad(batch_feat[:,f_ss_first_pad_left:max_flen].transpose(1,2), (0,f_es_first_pad_right-max_flen), "replicate").transpose(1,2)
                                i_in += 1
                                batch_feat_cv_in[i_cv_enc] = F.pad(batch_feat_cv_data[i_cv][:,f_ss_first_pad_left:max_flen].transpose(1,2), (0,diff_pad), "replicate").transpose(1,2)
                                i_cv_enc += 1
                                flag_cv = False
                        else:
                            if (i + 1) % 6 == 0:
                        ...
                        if flag_cv:
                            ...
                            if i % 3 == 2:
                                batch_feat_in[i_in] = F.pad(batch_feat[:,:max_flen].transpose(1,2), (-f_ss_first_pad_left,f_es_first_pad_right-max_flen), "replicate").transpose(1,2)
                                i_in += 1
                                batch_feat_cv_in[i_cv_enc] = F.pad(batch_feat_cv_data[i_cv][:,:max_flen].transpose(1,2), (-f_ss_first_pad_left,diff_pad), "replicate").transpose(1,2)
                                i_cv_enc += 1
                                flag_cv = False
                        else:
                            if (i + 1) % 6 == 0:
                ...
                for i in range(0,args.n_half_cyc,2):
                    ...
                    if i > 0:
                        ...
                        if post_pad_right > 0:
                            ...
                            cyc_rec_mcep = batch_mcep_rec[i_1][:,post_pad_left:]
                        _, qz_alpha[i], h_z[i] \
                            = model_encoder(torch.cat((batch_feat_in[i_in][:,:,:args.excit_dim], cyc_rec_mcep), 2), h=h_z[i], outpad_right=outpad_rights[idx_in], do=True)
                    else:
                        _, qz_alpha[i], h_z[i] = model_encoder(batch_feat_in[i_in], h=h_z[i], outpad_right=outpad_rights[idx_in], do=True)
                        batch_mcep_in_sc, h_mcep_in_sc = model_classifier(feat=batch_feat[:,:,args.excit_dim:], h=h_mcep_in_sc)
                    ## reconstruction and conversion
                    ...
                    batch_mcep_cv[i_cv], h_mcep_cv[i_cv] = model_decoder(batch_sc_cv_in[i_cv_in], z[i], h=h_mcep_cv[i_cv], outpad_right=outpad_rights[idx_in], do=True)
                    ## post
                    idx_in += 1
                    i_in += 1
                    i_cv_in += 1
                    batch_mcep_rec_post[i], h_mcep_post[i] = model_post(batch_sc_in[idx_in], batch_mcep_rec[i],
                                        e=batch_feat_in[i_in][:,:,:2], outpad_right=outpad_rights[idx_in], h=h_mcep_post[i], do=True)
                    batch_mcep_cv_post[i_cv], h_mcep_cv_post[i_cv] = model_post(batch_sc_cv_in[i_cv_in], batch_mcep_cv[i_cv],
                                        e=batch_feat_cv_in[i_cv_enc][:,:,:2], outpad_right=outpad_rights[idx_in], h=h_mcep_cv_post[i_cv], do=True)
                    feat_len = batch_mcep_rec_post[i].shape[1]
                    batch_mcep_rec_post[i] = batch_mcep_rec_post[i][:,outpad_lefts[idx_in]:feat_len-outpad_rights[idx_in]]
                    batch_mcep_cv_post[i_cv] = batch_mcep_cv_post[i_cv][:,outpad_lefts[idx_in]:feat_len-outpad_rights[idx_in]]
                    batch_mcep_rec_sc[i], h_mcep_sc[i] = model_classifier(feat=batch_mcep_rec_post[i], h=h_mcep_sc[i])
                    batch_mcep_cv_sc[i_cv], h_mcep_cv_sc[i_cv] = model_classifier(feat=batch_mcep_cv_post[i_cv], h=h_mcep_cv_sc[i_cv])
                    if args.n_half_cyc > 1:
                        ## cyclic reconstruction
                        ...
                        if post_pad_right > 0:
                            ...
                            cv_mcep = batch_mcep_cv[i_cv][:,post_pad_left:]
                        _, qz_alpha[j], h_z[j] \
                            = model_encoder(torch.cat((batch_feat_cv_in[i_cv_enc], cv_mcep), 2), h=h_z[j], outpad_right=outpad_rights[idx_in], do=True)
                        ...
                        i_in += 1
                        batch_mcep_rec_post[j], h_mcep_post[j] = model_post(batch_sc_in[idx_in], batch_mcep_rec[j],
                                            e=batch_feat_in[i_in][:,:,:2], outpad_right=outpad_rights[idx_in], h=h_mcep_post[j], do=True)
                        batch_mcep_rec_post[j] = batch_mcep_rec_post[j][:,outpad_lefts[idx_in]:batch_mcep_rec_post[j].shape[1]-outpad_rights[idx_in]]
                        batch_mcep_rec_sc[j], h_mcep_sc[j] = model_classifier(feat=batch_mcep_rec_post[j], h=h_mcep_sc[j])
            else:
                idx_in = 0
                i_in = 0
                ...
                for i in range(0,args.n_half_cyc,2):
                    ...
                    if i > 0:
                        ...
                        if post_pad_right > 0:
                            ...
                            cyc_rec_mcep = batch_mcep_rec[i_1][:,post_pad_left:]
                        _, qz_alpha[i], h_z[i] \
                            = model_encoder(torch.cat((batch_feat_in[i_in][:,:,:args.excit_dim], cyc_rec_mcep), 2), outpad_right=outpad_rights[idx_in], do=True)
                    else:
                        _, qz_alpha[i], h_z[i] = model_encoder(batch_feat_in[i_in], outpad_right=outpad_rights[idx_in], do=True)
                        batch_mcep_in_sc, h_mcep_in_sc = model_classifier(feat=batch_feat[:,:,args.excit_dim:])
                    ## reconstruction and conversion
                    eps = torch.empty_like(qz_alpha[i][:,:,:args.lat_dim])
                    eps.uniform_(eps_1,1)
                    ...
                    batch_mcep_cv[i_cv], h_mcep_cv[i_cv] = model_decoder(batch_sc_cv_in[i_cv_in], z[i], outpad_right=outpad_rights[idx_in], do=True)
                    ## post
                    idx_in += 1
                    i_in += 1
                    i_cv_in += 1
                    batch_mcep_rec_post[i], h_mcep_post[i] = model_post(batch_sc_in[idx_in], batch_mcep_rec[i],
                                        e=batch_feat_in[i_in][:,:,:2], outpad_right=outpad_rights[idx_in], do=True)
                    batch_mcep_cv_post[i_cv], h_mcep_cv_post[i_cv] = model_post(batch_sc_cv_in[i_cv_in], batch_mcep_cv[i_cv],
                                        e=batch_feat_cv_in[i_cv_enc][:,:,:2], outpad_right=outpad_rights[idx_in], do=True)
                    feat_len = batch_mcep_rec_post[i].shape[1]
                    batch_mcep_rec_post[i] = batch_mcep_rec_post[i][:,outpad_lefts[idx_in]:feat_len-outpad_rights[idx_in]]
                    batch_mcep_cv_post[i_cv] = batch_mcep_cv_post[i_cv][:,outpad_lefts[idx_in]:feat_len-outpad_rights[idx_in]]
                    batch_mcep_rec_sc[i], h_mcep_sc[i] = model_classifier(feat=batch_mcep_rec_post[i])
                    ...
                    if args.n_half_cyc > 1:
                        ...
                        if post_pad_right > 0:
                            ...
                            cv_mcep = batch_mcep_cv[i_cv][:,post_pad_left:]
                        _, qz_alpha[j], h_z[j] \
                            = model_encoder(torch.cat((batch_feat_cv_in[i_cv_enc], cv_mcep), 2), outpad_right=outpad_rights[idx_in], do=True)
                        eps = torch.empty_like(qz_alpha[j][:,:,:args.lat_dim])
                        eps.uniform_(eps_1,1)
                        z[j] = qz_alpha[j][:,:,:args.lat_dim] - torch.exp(qz_alpha[j][:,:,args.lat_dim:]) * eps.sign() * torch.log1p(-eps.abs()) # sampling laplace
                        idx_in += 1
            ...
            if len(idx_select) > 0:
                ...
                for j in range(len(idx_select)):
                    ...
                    for i in range(args.n_half_cyc):
                        mcep_est = batch_mcep_rec_post[i][k,:flens_utt]
                        if mcep_est.shape[0] > 1:
                            batch_loss_px_select += mcd_constant*(torch.mean(torch.sum(criterion_l1(mcep_est, powmcep), -1)) \
                                                        + torch.mean(torch.mean(criterion_l1(mcep_est, powmcep), -1)) \
                                                    + torch.sqrt(torch.mean(torch.sum(criterion_l2(mcep_est, powmcep), -1))) \
                                                        + torch.sqrt(torch.mean(torch.mean(criterion_l2(mcep_est, powmcep), -1))))
                        else:
                            batch_loss_px_select += mcd_constant*(torch.mean(torch.sum(criterion_l1(mcep_est, powmcep), -1)) \
                                                        + torch.mean(torch.mean(criterion_l1(mcep_est, powmcep), -1)))
                ...
                batch_loss += batch_loss_px_select + batch_loss_px_ms_norm_select + batch_loss_px_ms_err_select \
                                    + batch_loss_sc_mcep_in_kl_select + batch_loss_sc_mcep_kl_select
                if len(idx_select_full) > 0:
                    ...
                    for i in range(args.n_half_cyc):
                        batch_mcep_rec_post[i] = torch.index_select(batch_mcep_rec_post[i],0,idx_select_full)
                        batch_mcep_rec_sc[i] = torch.index_select(batch_mcep_rec_sc[i],0,idx_select_full)
                        if i % 2 == 0:
                            batch_mcep_cv_post[i//2] = torch.index_select(batch_mcep_cv_post[i//2],0,idx_select_full)
    ...
    torch.save({"model_encoder": model_encoder.state_dict(),
                "model_decoder": model_decoder.state_dict(),
                "model_classifier": model_decoder.state_dict(),
                "model_post": model_decoder.state_dict()}, args.expdir + "/checkpoint-final.pkl")
